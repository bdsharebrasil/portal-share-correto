...
type DocumentItem = (DocumentFolder & {
  type: 'folder';
}) | (Document & {
  type: 'file';
});
...
export default function Documentos() {
  ...
  useEffect(() => {
    const loadUsers = async () => {
      const {
        data,
        error
      } = await supabase.from("user_profiles").select("id, full_name").eq("employment_status", "ativo").order("full_name");
      if (!error && data) {
        setUsers(data);
      }
    };
  ...
  const loadFolderContents = useCallback(async (folderId: string) => {
    try {
      ...
      // Adicionar subpastas como projetos
      ...
      // Adicionar documentos como projetos
      (docs || []).forEach(doc => {
        projects.push({
          id: doc.id,
          title: doc.nome,
  ...
  const loadDocuments = useCallback(async () => {
    try {
      // Buscar pastas
      let foldersQuery = supabase.from("document_folders").select("*");
  ...
  const handleCreateFolder = async () => {
    ...
    if (isRestricted && selectedUsers.length > 0 && folderData) {
      ...
      const {
        error: permError
      } = await supabase.from("document_folder_permissions").insert(permissions);
      if (permError) {
  ...
  const handleDeleteDocument = async (item: DocumentItem) => {
    ...
    if (item.tipo === "file") {
      ...
      const {
        error
      } = await supabase.from("document_folders").delete().eq("id", item.id);
      if (error) {
        toast.error("Erro ao excluir");
        return;
      }
  ...
  return <Layout>
      <div className="p-6 space-y-6">
        ...
        <div className="flex flex-wrap gap-2">
          {currentFolder && <Button variant="outline" size="sm" onClick={handleGoBack}>
        ...
        {showNewFolder && <Card className="rounded-xl border-border/50">
            <CardContent className="pt-4">
              <div className="space-y-4">
                <div className="space-y-2">
                  <Label htmlFor="new-folder-name">Nome da Pasta</Label>
                  <Input id="new-folder-name" value={newFolderName} onChange={event => setNewFolderName(event.target.value)} placeholder="Digite o nome da pasta" />
                </div>
                
                {canCreateRestrictedFolder && <>
                    <div className="flex items-center gap-3 p-3 bg-muted/50 rounded-lg">
                      <Switch id="is-restricted" checked={isRestricted} onCheckedChange={setIsRestricted} />
                    ...
                    {isRestricted && <div className="space-y-2">
                        ...
                        <ScrollArea className="h-48 border rounded-lg p-3">
                          <div className="space-y-2">
                            {users.map(user => <div key={user.id} className="flex items-center gap-3 p-2 hover:bg-muted/50 rounded-md cursor-pointer" onClick={() => toggleUserSelection(user.id)}>
                        ...
                        {selectedUsers.length > 0 && <p className="text-xs text-muted-foreground">
        ...
        {showUpload && <Card className="rounded-xl border-border/50">
            <CardContent className="pt-4">
              <div className="space-y-3">
                <div className="space-y-2">
                  <Label htmlFor="upload-file">Arquivo</Label>
                  <Input id="upload-file" type="file" onChange={event => setUploadFile(event.target.files?.[0] ?? null)} />
                ...
                <div className="space-y-2">
                  ...
                  <Textarea id="upload-caption" value={uploadCaption} onChange={event => setUploadCaption(event.target.value)} placeholder="Adicione uma descrição para o arquivo" rows={2} />
                </div>
                <div className="flex flex-wrap gap-2">
          ...
          {items.some(item => item.tipo === 'folder') && <div>
              ...
              <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {items.filter(item => item.tipo === 'folder').map(item => <div key={item.id} className="relative group">
                      ...
                      {item.is_restricted && <Lock className="h-4 w-4 absolute top-2 right-10 text-amber-500" />}
                    </div>)}
              </div>
            </div>}
                ...
                {items.filter(item => item.tipo === 'file').map(item => <Card key={item.id} className="transition-colors hover:bg-accent/50 rounded-xl">
                      <CardContent className="p-4 space-y-4">
                        ...
                        <div className="flex justify-end gap-1">
                          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleViewFile(item)}>
                            <Eye className="h-4 w-4" />
                          </Button>
                          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleDownloadFile(item)}>
                            <Download className="h-4 w-4" />
                          </Button>
                          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => handleDeleteDocument(item)}>
        ...
        {items.length === 0 && <div className="py-12 text-center text-muted-foreground">
