  ...
  return (
    <Layout>
      <div className="space-y-8 max-w-[1800px] mx-auto pb-20">
        ...
        {/* CABEÇALHO */}
        <div className="bg-gradient-to-r from-card/80 to-background/80 border border-border/50 p-8 rounded-3xl shadow-xl">
          <div className="flex items-center gap-6">
            <button 
              ...
              className="p-3 bg-background hover:bg-card-secondary rounded-xl transition-all border border-border"
            >
          ...
          <>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {lendersData.length === 0 ? (
                <div className="lg:col-span-3 flex items-center justify-center h-64 bg-card/40 border border-border/50 rounded-3xl">
                  <p className="text-muted-foreground text-center">Nenhum empréstimo registrado para esta aeronave</p>
                </div>
              ) : (
                lendersData.map(lender => (
                  ...
                  >
                    <div className="space-y-4">
                      ...
                      <p className="text-muted-foreground text-sm">Clique para ver detalhes dos empréstimos</p>
                    </div>
                  </button>
                ))
              )}
        ...
        {view === 'borrowers' && selectedLenderId && (
          <>
            <div className="bg-gradient-to-br from-card/40 to-background/40 border border-border/50 p-6 rounded-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {borrowersForLender.length === 0 ? (
                <div className="lg:col-span-3 flex items-center justify-center h-64 bg-card/40 border border-border/50 rounded-3xl">
                  <p className="text-muted-foreground text-center">Nenhum empréstimo registrado para este sócio</p>
                </div>
              ) : (
                    ...
                    >
                      <div className="space-y-6">
                        ...
                        <div>
                          ...
                          <p className="text-[9px] text-muted-foreground font-bold uppercase mt-1">Cliente Mutuário</p>
                        </div>

                        {/* GRID 2x2 DE MÉTRICAS */}
                        <div className="grid grid-cols-2 gap-4">
                          <div className="bg-background/40 p-4 rounded-2xl border border-border/30">
                            <p className="text-[8px] font-black text-muted-foreground uppercase mb-2">Emprestado</p>
                            <p className="text-xl font-black text-sky-500 font-mono">{decimalToHM(borrower.total_borrowed)}</p>
                          </div>
                          <div className="bg-background/40 p-4 rounded-2xl border border-border/30">
                            <p className="text-[8px] font-black text-muted-foreground uppercase mb-2">Devolvido</p>
                        ...
                        <div>
                          <div className="flex items-center justify-between mb-2">
                            <p className="text-[9px] font-black text-muted-foreground uppercase">Devolvido</p>
                            <p className="text-[9px] font-black text-muted-foreground">{percentPaidBack.toFixed(0)}%</p>
                          </div>
                          <div className="w-full bg-card-secondary/50 rounded-full h-2.5 overflow-hidden border border-border/30">
                            <div
        ...
        {view === 'details' && selectedLenderId && selectedBorrowerId && (
          <>
            ...
            {loansForBorrower.length > 0 && (
              <div className="bg-card/40 border border-border/50 rounded-3xl overflow-hidden shadow-xl">
                      ...
                      {loansForBorrower.map(loan => {
                        ...
                        return (
                          <tr key={loan.id} className="hover:bg-card-secondary/20 transition-colors">
                            <td className="px-4 py-4 text-muted-foreground whitespace-nowrap">{formattedDate}</td>
                            <td className="px-4 py-4 text-muted-foreground font-mono">{trecho}</td>
                            ...
                            <td className="px-4 py-4 text-center text-muted-foreground font-mono whitespace-nowrap">{fuelAdded}</td>
                            <td className="px-4 py-4 text-muted-foreground truncate text-xs">{picName}</td>
