  ...
  const loadAircraft = async () => {
    try {
      const { data, error } = await supabase
        .from("aircraft")
        .select("id, registration, model, status, image_url, cell_hours_current")
        .order("registration");
    ...
    <Layout>
      <div className="p-6 space-y-8">
        ...
        <motion.div 
          ...
          className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/20 via-primary/10 to-background p-8 border border-primary/20"
        >
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
              <AnimatePresence>
                {activeAircraft.map((ac, index) => (
                  ...
                  >
                    <Card 
                      className="group cursor-pointer overflow-hidden border-border/50 hover:border-primary/50 hover:shadow-lg hover:shadow-primary/10 transition-all duration-300 bg-gradient-to-br from-background to-muted/30"
                        ...
                        <div className="p-4 space-y-3">
                          ...
                          <div className="flex items-center justify-between pt-2 border-t border-border/50">
                            <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                              ...
                              <span>{ac.cell_hours_current?.toFixed(1) || "0"}h célula</span>
                            </div>
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <motion.div
              animate={{ rotate: showInactive ? 180 : 0 }}
                ...
                {inactiveAircraft.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {inactiveAircraft.map((ac, index) => (
                      ...
                      >
                        <Card 
                          className="cursor-pointer border-border/30 hover:border-orange-500/50 hover:shadow-lg hover:shadow-orange-500/10 transition-all duration-300 opacity-70 hover:opacity-100 bg-muted/20"
                          onClick={() => handleSelectAircraft(ac.id)}
                        >
