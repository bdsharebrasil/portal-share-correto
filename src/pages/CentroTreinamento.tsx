...
export default function CentroTreinamento() {
  ...
  return <Layout>
    <div className="mx-auto max-w-6xl space-y-8 px-4 py-6">
      ...
      <section><div className="mb-4 flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">Agenda de reuniões</p><h2 className="mt-1 text-xl font-semibold">Próximas salas</h2><p className="mt-1 text-sm text-muted-foreground">Todos os usuários podem entrar e participar. Apenas admin e gestor master criam reuniões.</p></div>{canCreate && <Button onClick={() => setDialogOpen(true)}><Plus className="size-4" /> Criar reunião</Button>}</div>
