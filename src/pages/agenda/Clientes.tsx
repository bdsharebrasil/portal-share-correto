...
const PANEL = "bg-card/50 border border-border/80 rounded-xl p-5 sm:p-6 space-y-4";
const LABEL = "text-muted-foreground font-medium mb-2 block text-sm";
...
export default function Clientes() {
  ...
  const ClienteRow = ({ cliente }: { cliente: Cliente }) => (
    <button
      ...
      className="w-full p-4 flex items-center gap-4 hover:bg-card-secondary/60 transition-colors text-left group first:rounded-t-xl last:rounded-b-xl"
    >
      ...
      <div className="flex-1 min-w-0">
        <h3 className="font-semibold text-foreground group-hover:text-cyan-400 transition-colors truncate">{cliente.razao_social}</h3>
        {cliente.aeronave_ownerships && cliente.aeronave_ownerships.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mt-1.5">
            {cliente.aeronave_ownerships.map((ownership, idx) => (
              <Badge key={idx} variant="secondary" className="text-xs font-medium px-2 py-0.5 bg-card-secondary text-muted-foreground border border-border/60 rounded-md">
      ...
      <ChevronLeft className="h-4 w-4 rotate-180 text-muted-foreground group-hover:text-cyan-400 transition-colors flex-shrink-0" />
    </button>
  );
    ...
    <>
        ...
        <div className="p-4 sm:p-6 space-y-6 max-w-7xl mx-auto">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2">
              <div className="flex border border-border bg-card/60 rounded-lg p-1">
                <Button
                  onClick={() => handleViewModeChange('card')}
                  variant="ghost"
                  size="sm"
                  className={`h-9 w-9 p-0 rounded-md ${viewMode === 'card' ? 'bg-card-secondary text-cyan-400' : 'text-muted-foreground hover:text-foreground hover:bg-card-secondary/60'}`}
                  title="Cards"
                >
                ...
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  className={`h-9 w-9 p-0 rounded-md ${viewMode === 'list' ? 'bg-card-secondary text-cyan-400' : 'text-muted-foreground hover:text-foreground hover:bg-card-secondary/60'}`}
                  title="Lista"
                >
          ...
          <div className="flex items-stretch gap-4">
            <Card
              ...
              className={`mx-[52px] flex min-h-0 items-stretch justify-end overflow-auto px-[6px] cursor-pointer transition-all duration-200 rounded-xl bg-card/50 border ${showInactive ? 'border-amber-500/50 shadow-lg shadow-amber-500/5' : 'border-border/80 hover:border-border'}`}
            >
          ...
          {loading ? (
            ...
            <Card className="border-dashed border-border bg-card/30 rounded-xl">
              <CardContent className="flex flex-col items-center justify-center py-16 text-center">
                <div className="h-16 w-16 rounded-xl bg-card-secondary/70 flex items-center justify-center mb-4">
                ...
                <Button onClick={() => handleOpenDialog()} variant="outline" className="gap-2 rounded-lg bg-card border-border text-foreground hover:bg-card-secondary hover:text-foreground">
                  <Plus className="h-4 w-4" />
                  Cadastrar Cliente
                </Button>
              </CardContent>
            ...
            <div className="space-y-6">
              ...
              {!showInactive && activeClientes.length > 0 && viewMode === 'list' && (
                <div className="border border-border/80 bg-card/40 rounded-xl divide-y divide-border/80 overflow-hidden">
              ...
              {showInactive && inactiveClientes.length > 0 && (
                <div className="space-y-4">
                  <div className="flex items-center gap-3">
                    ...
                    <Badge variant="secondary" className="bg-card-secondary text-muted-foreground border border-border/60 rounded-md">{inactiveClientes.length}</Badge>
                  </div>
                  {viewMode === 'card' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      {viewingCliente && (
        <div className="p-4 sm:p-6 space-y-6 max-w-5xl mx-auto">
          <Button variant="ghost" onClick={handleCloseView} className="gap-2 text-muted-foreground hover:text-foreground hover:bg-card-secondary/60 -ml-2 rounded-lg">
            <ChevronLeft className="h-4 w-4" />
            Voltar para Clientes
          </Button>

          <Card className="overflow-hidden border border-border/80 shadow-xl rounded-2xl">
            <div className="relative bg-gradient-to-br from-card via-card to-background">
              ...
              <CardContent className="relative p-6 sm:p-8">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-6">
                  <div className="flex items-center gap-5">
                    <Avatar className="h-20 w-20 sm:h-24 sm:w-24 ring-4 ring-cyan-400/30 shadow-xl bg-card-secondary flex items-center justify-center flex-shrink-0">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-2.5 flex-wrap">
                        ...
                        <Badge variant="outline" className="font-mono text-xs bg-card-secondary/60 border-border rounded-md text-muted-foreground">{viewingCliente.cnpj}</Badge>
          ...
          {(viewingCliente.telefone || viewingCliente.email || viewingCliente.endereco || viewingCliente.cidade || viewingCliente.uf || viewingCliente.contato_financeiro) && (
            <Card className="bg-card/50 border border-border/80 rounded-xl">
              ...
              <CardContent>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                  {viewingCliente.telefone && (
                    <div>
                      <p className="text-xs text-muted-foreground mb-2 flex items-center gap-1.5"><Phone className="h-3 w-3" />Telefone</p>
                      <p className="text-sm font-medium text-foreground">{viewingCliente.telefone}</p>
                    </div>
                  )}
                  {(viewingCliente.email || (viewingCliente.emails || []).length > 0) && (
                    <div>
                      <p className="text-xs text-muted-foreground mb-2 flex items-center gap-1.5"><Mail className="h-3 w-3" />E-mail</p>
                      {Array.from(new Set([viewingCliente.email, ...(viewingCliente.emails || [])].filter(Boolean) as string[])).map((mail) => (
                        <p key={mail} className="text-sm font-medium text-foreground break-all">{mail}</p>
                      ))}
                    </div>
                  )}
                  {(viewingCliente.endereco || viewingCliente.cidade || viewingCliente.uf) && (
                    <div>
                      <p className="text-xs text-muted-foreground mb-2 flex items-center gap-1.5"><MapPin className="h-3 w-3" />Endereço</p>
                      <p className="text-sm font-medium text-foreground">{[viewingCliente.endereco, [viewingCliente.cidade, viewingCliente.uf].filter(Boolean).join(' - ')].filter(Boolean).join(', ')}</p>
                    </div>
                  )}
                  {viewingCliente.contato_financeiro && (
                    <div>
                      <p className="text-xs text-muted-foreground mb-2 flex items-center gap-1.5"><Wallet className="h-3 w-3" />Contato Financeiro</p>
                      <p className="text-sm font-medium text-foreground">{viewingCliente.contato_financeiro}</p>
                    </div>
                  )}
                </div>
              </CardContent>
            ...
            <Card className="bg-card/50 border border-border/80 rounded-xl">
              ...
              <CardContent className="space-y-5">
                ...
                {viewingCliente.aeronave_ownerships && viewingCliente.aeronave_ownerships.length > 0 && (
                  <div>
                    <p className="text-xs text-muted-foreground mb-3 flex items-center gap-1.5"><Plane className="h-3 w-3" />Aeronaves e Participação</p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                      {viewingCliente.aeronave_ownerships.map((ownership, idx) => (
                        <div key={idx} className="flex justify-between items-center p-3 bg-background/50 border border-border/80 rounded-lg">
                          <span className="text-sm font-medium text-foreground">{ownership.aircraft_registration} - {ownership.aircraft_model}</span>
                          <Badge variant="secondary" className="bg-card-secondary text-foreground border border-border/60 rounded-md font-semibold">{ownership.ownership_percentage}%</Badge>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                {viewingCliente.tem_socio && viewingCliente.partners && viewingCliente.partners.length > 0 && (
                  <div>
                    <p className="text-xs text-muted-foreground mb-3 flex items-center gap-1.5 font-medium"><Users className="h-3 w-3" />Sócios / Cotistas</p>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      {viewingCliente.partners.map((partner, idx) => (
                        <div key={idx} className="p-3 bg-background/50 border border-border/80 rounded-lg">
                ...
                {viewingCliente.observacoes && (
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground whitespace-pre-wrap bg-background/50 border border-border/80 p-3 rounded-lg">{viewingCliente.observacoes}</p>
                  </div>
                )}
              </CardContent>
            </Card>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {viewingCliente.documents.map((doc, index) => (
                    <button
                      ...
                      className="p-4 border border-border/80 bg-background/40 rounded-lg hover:bg-card-secondary/50 hover:border-cyan-500/40 transition-all text-left group flex items-center gap-3"
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-medium text-foreground truncate group-hover:text-cyan-400 transition-colors">{getDocumentTitle(doc)}</p>
                        <p className="text-xs text-muted-foreground">{doc.legenda ? `${doc.nome || 'Documento'} · ` : ""}{doc.enviado_em ? new Date(doc.enviado_em).toLocaleDateString('pt-BR') : ""}</p>
                      </div>
                    </button>
                  ))}
                </div>
      ...
      <Dialog open={!!previewDoc} onOpenChange={open => { if (!open) setPreviewDoc(null); }}>
        <DialogContent className="max-w-5xl w-[95vw] h-[85vh] p-0 overflow-hidden bg-background border border-border rounded-xl">
            ...
            {previewDoc && isPdfDocument(previewDoc) ? (
              ...
              <div className="p-6 space-y-3">
                ...
                {previewDoc?.url ? (
                  ...
                  <p className="text-sm text-muted-foreground">Arquivo sem URL disponível.</p>
                )}
              </div>
            )}
          </div>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-4xl w-[95vw] sm:w-full max-h-[90vh] overflow-y-auto bg-background border border-border rounded-xl p-4 sm:p-6">
          ...
          <Tabs defaultValue="info" className="w-full mt-4">
            <TabsList className="grid w-full grid-cols-2 bg-card/70 border border-border rounded-lg p-1 h-auto">
              <TabsTrigger
                value="info"
                className="gap-2 py-2 data-[state=active]:bg-gradient-to-r data-[state=active]:from-cyan-500 data-[state=active]:to-blue-500 data-[state=active]:text-white data-[state=active]:shadow-none text-muted-foreground rounded-md transition-all"
              >
            ...
            <TabsContent value="info" className="space-y-5 mt-4">
              <div className={PANEL}>
                <div className={EYEBROW}>
                  ...
                  <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Informações Principais</h3>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-2">
                      <Label htmlFor="email" className="text-muted-foreground font-medium flex items-center gap-1.5 text-sm"><Mail className="h-3.5 w-3.5" />E-mail</Label>
              ...
              <div className={PANEL}>
                <div className="flex items-center justify-between flex-wrap gap-2">
                  <div className={EYEBROW + " mb-0"}>
                    <span className="h-1.5 w-1.5 rounded-full bg-cyan-400" />
                    <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5"><Plane className="h-3.5 w-3.5" />Aeronaves</h3>
                  </div>
                  <div className="flex gap-2">
                    {aircraftOptions.length === 0 && (
                      <Button type="button" variant="outline" size="sm" onClick={loadAircraftOptions} disabled={loadingAircraft} className="gap-1.5 bg-background/60 border-border hover:bg-card-secondary text-muted-foreground rounded-lg">
                        <RefreshCw className="h-3.5 w-3.5" /> Recarregar
                      </Button>
                    )}
                    <Button type="button" variant="outline" size="sm" onClick={addAircraftOwnership} disabled={aircraftOptions.length === 0 || loadingAircraft} className="gap-1.5 bg-background/60 border-border hover:bg-card-secondary text-muted-foreground rounded-lg">
                ...
                {aircraftOwnerships.length === 0 ? (
                  <div className="p-4 border border-dashed border-border rounded-lg text-center text-muted-foreground">Nenhuma aeronave adicionada</div>
                ) : (
                  <div className="space-y-3">
                    {aircraftOwnerships.map((ownership, index) => (
                      <div key={index} className="space-y-3 p-4 border border-border rounded-lg bg-background/40">
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex-1">
                            <Label className={LABEL}>Aeronave *</Label>
                            {loadingAircraft ? (
                              <div className="p-3 border border-border rounded-lg text-center text-sm text-muted-foreground bg-background/60">Carregando aeronaves...</div>
                            ) : aircraftOptions.length === 0 ? (
                              ...
                              <Select value={ownership.aircraft} onValueChange={v => updateAircraftOwnership(index, 'aircraft', v)}>
                                ...
                                <SelectContent className="max-h-64 bg-card border-border rounded-lg">
                                  {aircraftOptions.map(a => (
                                    <SelectItem key={a.id} value={a.id} className="text-foreground rounded-md focus:bg-card-secondary">
                                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <Label className="text-muted-foreground mb-1.5 block text-xs flex items-center gap-1.5"><Percent className="h-3 w-3" />Percentual de Participação — aceita decimais</Label>
                          <Input
                ...
                {aircraftOwnerships.length > 0 && (
                  <div className="p-3.5 bg-background/50 rounded-lg border border-border">
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-muted-foreground">Total de Participação:</span>
              ...
              <div className={PANEL}>
                <div className={EYEBROW}>
                  <span className="h-1.5 w-1.5 rounded-full bg-cyan-400" />
                  <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider">Status e Observações</h3>
                </div>
                <div className="space-y-4">
                  <div>
                    ...
                    <Select value={formData.status} onValueChange={v => setFormData({ ...formData, status: v })}>
                      ...
                      <SelectContent className="bg-card border-border rounded-lg">
                        <SelectItem value="ativo" className="text-foreground rounded-md focus:bg-card-secondary">
                          <span className="flex items-center gap-2"><span className="h-1.5 w-1.5 rounded-full bg-emerald-400" />Ativo</span>
                        </SelectItem>
                        <SelectItem value="inativo" className="text-foreground rounded-md focus:bg-card-secondary">
              ...
              <div className={PANEL}>
                <div className={EYEBROW}>
                  <span className="h-1.5 w-1.5 rounded-full bg-cyan-400" />
                  <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5"><Users className="h-3.5 w-3.5" />Sócios / Cotistas</h3>
                </div>
                <div className="space-y-4">
                  <div>
                  ...
                  {hasPartner && (
                    <div className="space-y-3 mt-2">
                      {editingCliente?.tem_socio && editingCliente.partners && editingCliente.partners.length > 0 && (
                        <div className="p-3.5 bg-background/60 border border-border rounded-lg">
                          ...
                          <div className="space-y-1.5">
                            {editingCliente.partners.map((savedPartner, idx) => (
                              <div key={idx} className="text-xs text-muted-foreground p-2 bg-card/70 rounded-md">
                                <span className="font-medium text-muted-foreground">{savedPartner.nome}</span> · <span className="font-mono">{savedPartner.cpf}</span>
                      ...
                      <p className="text-sm text-muted-foreground">Adicione os sócios/cotistas da empresa</p>

                      {partners.length === 0 ? (
                        <div className="p-4 border border-dashed border-border rounded-lg text-center text-muted-foreground">Nenhum sócio adicionado</div>
                      ) : (
                        <div className="space-y-3">
                            ...
                            <div key={index} className="space-y-3 p-4 border border-border rounded-lg bg-background/40">
                              <div className="flex items-start justify-between gap-2">
                                <div className="flex-1 grid grid-cols-2 md:grid-cols-4 gap-3">
                                  <div>
                                    <Label className="text-muted-foreground mb-1.5 block text-xs">Nome</Label>
                                    <Input value={partner.nome} onChange={e => updatePartner(index, 'nome', e.target.value)} placeholder="Nome do sócio" className={FIELD} />
                                  </div>
                                  <div>
                                    <Label className="text-muted-foreground mb-1.5 block text-xs">CPF/CNPJ</Label>
                                  ...
                                  <div>
                                    <Label className="text-muted-foreground mb-1.5 block text-xs">% Participação</Label>
                                  ...
                                  <div>
                                    <Label className="text-muted-foreground mb-1.5 block text-xs">Código</Label>
                      ...
                      <Button type="button" variant="outline" onClick={addPartner} className="w-full gap-2 bg-background/60 border-border hover:bg-card-secondary text-cyan-400 hover:text-cyan-300 rounded-lg">
                        <Plus className="h-4 w-4" />
                        Adicionar Sócio
                      </Button>
                    </div>
            ...
            <TabsContent value="files" className="space-y-5 mt-4">
              <div className={PANEL}>
                <div className={EYEBROW}>
                  <span className="h-1.5 w-1.5 rounded-full bg-cyan-400" />
                  <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5"><ImageIcon className="h-3.5 w-3.5" />Logo da Empresa</h3>
                </div>
                <p className="text-sm text-muted-foreground">Envie a logo da empresa (formatos: PNG, JPG, SVG)</p>
                <div className="flex flex-col sm:flex-row items-center gap-6">
                  {logoPreview && (
                    <Avatar className="h-24 w-24 border border-border ring-2 ring-cyan-400/20 flex-shrink-0">
                      <AvatarImage src={logoPreview} alt="Logo" className="object-contain" />
                      <AvatarFallback className="bg-card"><ImageIcon className="h-8 w-8 text-muted-foreground" /></AvatarFallback>
                    </Avatar>
                  )}
                  <div className="flex-1 w-full">
                    <label className="block cursor-pointer">
                      <div className="border-2 border-dashed border-border rounded-xl p-6 hover:border-cyan-500/50 hover:bg-card/30 transition-colors text-center">
                        <ImageIcon className="h-8 w-8 mx-auto mb-2 text-muted-foreground" />
                        <p className="text-sm text-muted-foreground font-medium">Clique para upload</p>
              ...
              <div className={PANEL}>
                <div className={EYEBROW}>
                  <span className="h-1.5 w-1.5 rounded-full bg-cyan-400" />
                  <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider flex items-center gap-1.5"><FileText className="h-3.5 w-3.5" />Documentos</h3>
                </div>
                <p className="text-sm text-muted-foreground">Anexe documentos como CNPJ, contratos, certificados, etc.</p>
                <label className="block cursor-pointer">
                  <div className="border-2 border-dashed border-border rounded-xl p-8 hover:border-cyan-500/50 hover:bg-card/30 transition-colors text-center">
                    <FileText className="h-8 w-8 mx-auto mb-2 text-muted-foreground" />
                    <p className="text-sm text-muted-foreground font-medium">Clique para upload ou arraste arquivos</p>
                    <p className="text-xs text-muted-foreground mt-1">Todos os formatos são suportados</p>
                  </div>
                ...
                {documentFiles.length > 0 && (
                  <div className="space-y-3 mt-2">
                    <h4 className="text-sm font-semibold text-muted-foreground">Novos documentos</h4>
                    <div className="space-y-2">
                      {documentFiles.map((file, index) => (
                        <div key={index} className="flex items-center justify-between p-3 border border-border rounded-lg bg-background/40 hover:bg-card/60 transition-colors">
                          ...
                          <Button variant="ghost" size="sm" onClick={() => removeDocument(index)} className="h-8 w-8 p-0 text-muted-foreground hover:text-rose-400 hover:bg-rose-500/10 rounded-lg flex-shrink-0">
                            <X className="h-4 w-4" />
                          </Button>
                        </div>
                      ))}
                ...
                {editingCliente?.documents && editingCliente.documents.length > 0 && (
                  <div className="space-y-3 mt-4 pt-5 border-t border-border">
                    <h4 className="text-sm font-semibold text-muted-foreground">Documentos existentes</h4>
                    <div className="space-y-2">
                      {editingCliente.documents.map((doc, index) => (
                        <div key={index} className="flex items-center justify-between p-3 border border-border rounded-lg bg-background/40 hover:bg-card/60 transition-colors">
                          ...
                          <Button type="button" variant="ghost" size="sm" onClick={() => removeDocumentFromEditing(index)} className="h-8 w-8 p-0 text-muted-foreground hover:text-rose-400 hover:bg-rose-500/10 rounded-lg flex-shrink-0">
                            <X className="h-4 w-4" />
                          </Button>
                        </div>
                      ))}
          ...
          <DialogFooter className="mt-6 pt-5 border-t border-border flex flex-col-reverse sm:flex-row gap-3 sm:justify-end">
            <Button variant="outline" onClick={handleCloseDialog} className="bg-card border-border text-muted-foreground hover:bg-card-secondary hover:text-foreground rounded-lg">
              Cancelar
            </Button>
            <Button
              onClick={handleSave}
      ...
      <AlertDialog open={!!deleteId} onOpenChange={() => setDeleteId(null)}>
        <AlertDialogContent className="bg-background border border-border rounded-xl">
          <AlertDialogHeader>
          ...
          <AlertDialogFooter className="gap-2">
            <AlertDialogCancel className="bg-card border-border text-muted-foreground hover:bg-card-secondary hover:text-foreground rounded-lg">
              Cancelar
            </AlertDialogCancel>
