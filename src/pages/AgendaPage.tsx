...
export default function AgendaPage() {
  ...
  const loadColaboradores = useCallback(async () => {
    try {
      const {
        data: profiles,
        error: profilesError
      } = await supabase.from("user_profiles").select("id,full_name,phone,email,created_at").order("full_name", {
        ascending: true
      });
      ...
      const colaboradoresData = (profiles ?? []).map(profile => ({
        id: profile.id,
  ...
  const loadContacts = useCallback(async (term?: string) => {
    try {
      ...
      const contactsQuery = supabase.from("contacts").select("id,name,phone,email,company_name,position,notes,created_at,updated_at,category,address,city").order("name", {
        ascending: true
      });
      if (likePattern) {
        contactsQuery.or(`name.ilike.${likePattern},company_name.ilike.${likePattern},phone.ilike.${likePattern},email.ilike.${likePattern},city.ilike.${likePattern}`);
      }
      const hotelsQuery = supabase.from("hoteis").select("*").order("nome", {
        ascending: true
      });
      if (likePattern) {
  ...
  const handleSearch = async (term: string) => {
    ...
    if (term.trim()) {
      ...
      const filtered = colaboradoresAll.filter(c => c.nome.toLowerCase().includes(normalizedTerm) || c.telefone?.toLowerCase().includes(normalizedTerm) || c.email?.toLowerCase().includes(normalizedTerm));
  ...
  const handleDeleteContact = async (contact: any) => {
    try {
      if (contact.origin === "hoteis") {
        const {
          error
        } = await supabase.from("hoteis").delete().eq("id", contact.id);
  ...
  const filteredContacts = activeTab === "colaboradores" ? colaboradores : contacts.filter((c: any) => c.categoria === activeTab);
  ...
  return <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          ...
          {activeTab === "hoteis" ? <Button onClick={handleAddHotel} className="flex items-center gap-2">
              <Plus className="h-4 w-4" />
              Adicionar Hotel
            </Button> : <Button onClick={handleAddContact} className="flex items-center gap-2">
              <Plus className="h-4 w-4" />
              Adicionar Contato
            </Button>}
        </div>
        ...
        <div className="relative max-w-md">
          ...
          <Input placeholder="Buscar por nome ou empresa..." value={searchTerm} onChange={e => handleSearch(e.target.value)} className="pl-10" />
        </div>

        <Tabs value={activeTab} onValueChange={v => setActiveTab(v as typeof activeTab)} className="w-full">
          <TabsList className="mt-2 w-full grid grid-cols-4 gap-4 bg-transparent p-0 border-0">
            <TabsTrigger value="clientes" className="rounded-2xl overflow-hidden transition-all p-0 data-[state=inactive]:hover:shadow-lg data-[state=active]:ring-2 data-[state=active]:ring-blue-500 data-[state=active]:ring-offset-2 data-[state=active]:ring-offset-background">
              <div className="w-full bg-gradient-to-br from-blue-500/10 to-cyan-500/5 border rounded-2xl overflow-hidden p-5 flex flex-col items-center gap-3 border-transparent bg-transparent">
            ...
            <TabsTrigger value="colaboradores" className="rounded-2xl overflow-hidden transition-all p-0 data-[state=inactive]:hover:shadow-lg data-[state=active]:ring-2 data-[state=active]:ring-emerald-500 data-[state=active]:ring-offset-2 data-[state=active]:ring-offset-background">
              <div className="w-full bg-gradient-to-br from-emerald-500/10 to-teal-500/5 border border-emerald-500/20 rounded-2xl overflow-hidden p-5 flex flex-col items-center gap-3 bg-transparent py-[18px]">
            ...
            <TabsTrigger value="fornecedores" className="rounded-2xl overflow-hidden transition-all p-0 data-[state=inactive]:hover:shadow-lg data-[state=active]:ring-2 data-[state=active]:ring-orange-500 data-[state=active]:ring-offset-2 data-[state=active]:ring-offset-background">
              <div className="w-full bg-gradient-to-br from-orange-500/10 to-amber-500/5 border border-orange-500/20 rounded-2xl overflow-hidden p-5 flex flex-col items-center gap-3 bg-transparent">
            ...
            <TabsTrigger value="hoteis" className="rounded-2xl overflow-hidden transition-all p-0 data-[state=inactive]:hover:shadow-lg data-[state=active]:ring-2 data-[state=active]:ring-violet-500 data-[state=active]:ring-offset-2 data-[state=active]:ring-offset-background">
        ...
        <div className="w-full mt-6">
            {loading ? <div className="flex items-center justify-center py-12">
              ...
              </div> : filteredContacts.length === 0 ? <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              if (isHotel) {
                return <Card key={contact.id} className="hover:shadow-lg transition-shadow group border border-white/30 bg-slate-900/30">
                              ...
                              <div className="space-y-4">
                                ...
                                <div className="flex items-start justify-between gap-4">
                                  <div className="flex-1">
                                    ...
                                    {contact.cidade && <div className="ml-8">
                                        <p className="text-sm font-semibold text-cyan-400 uppercase tracking-wide">
                                          📍 {contact.cidade}
                                        </p>
                                      </div>}
                                    {contact.telefone && <p className="text-xs text-slate-400 mt-2 ml-8">
                                        📞 {contact.telefone}
                                      </p>}
                                  </div>
                                  ...
                                  <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0">
                                    <Button variant="ghost" size="sm" onClick={() => handleEditContact(contact)} className="h-8 w-8 p-0 hover:bg-slate-800">
                                      <Edit className="h-4 w-4" />
                                    </Button>
                                    <AlertDialog>
                                      <AlertDialogTrigger asChild>
                                        <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-destructive hover:text-destructive hover:bg-slate-800">
                                      ...
                                      <AlertDialogContent>
                                        ...
                                        <AlertDialogFooter>
                                          ...
                                          <AlertDialogAction onClick={() => handleDeleteContact(contact)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
                                            Excluir
                                          </AlertDialogAction>
                                        </AlertDialogFooter>
                                      </AlertDialogContent>
                                ...
                                {/* Address */}
                                {contact.endereco && <div className="border-t border-white/20 pt-3">
                                ...
                                {/* Prices Section */}
                                {contact.precoSingle !== undefined && contact.precoSingle !== null || contact.precoDuplo !== undefined && contact.precoDuplo !== null ? <div className="border-t border-white/20 pt-4">
                                    <p className="text-xs text-slate-400 mb-3 font-semibold uppercase">Diárias</p>
                                    <div className="grid grid-cols-2 gap-4">
                                      {contact.precoSingle !== undefined && contact.precoSingle !== null && <div className="bg-slate-800/50 rounded-lg p-3 border border-cyan-500/30">
                                          <p className="text-xs text-slate-400 mb-1 font-medium">Single</p>
                                          <p className="text-lg font-bold text-cyan-400">
                                            {currencyFormatter.format(contact.precoSingle)}
                                          </p>
                                        </div>}
                                      {contact.precoDuplo !== undefined && contact.precoDuplo !== null && <div className="bg-slate-800/50 rounded-lg p-3 border border-cyan-500/30">
              ...
              return <ContactCard key={contact.id} contact={contact} onEdit={handleEditContact} onDelete={handleDeleteContact} />;
            })}
                  </div>
                </div>
              </div>}
        ...
        <ContactModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} contact={editingContact} onSave={handleSaveContact} onUpdate={handleUpdateContact} />
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input id="hotel_nome" value={hotelForm.nome} onChange={e => setHotelForm({
              ...hotelForm,
              nome: e.target.value
            })} />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input id="hotel_telefone" value={hotelForm.telefone} onChange={e => setHotelForm({
                ...hotelForm,
                ...
                <div>
                  ...
                  <Input id="hotel_cidade" value={hotelForm.cidade} onChange={e => setHotelForm({
                ...hotelForm,
              ...
              <div>
                ...
                <Input id="hotel_endereco" value={hotelForm.endereco} onChange={e => setHotelForm({
              ...hotelForm,
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input id="hotel_preco_single" type="number" step="0.01" value={hotelForm.preco_single} onChange={e => setHotelForm({
                ...hotelForm,
                preco_single: e.target.value
              })} />
                </div>
                <div>
                  <Label htmlFor="hotel_preco_duplo">Preço Duplo</Label>
                  <Input id="hotel_preco_duplo" type="number" step="0.01" value={hotelForm.preco_duplo} onChange={e => setHotelForm({
                ...hotelForm,
                preco_duplo: e.target.value
              })} />
                </div>
              </div>
            </div>
            ...
            try {
              ...
              if (editingHotel) {
                const {
                  error
                } = await supabase.from("hoteis").update(payload).eq("id", editingHotel.id);
                if (error) throw error;
                toast({
                  title: "Sucesso",
              ...
              toast({
                title: "Erro",
                description: "Erro ao salvar hotel",
                variant: "destructive"
              });
            }
