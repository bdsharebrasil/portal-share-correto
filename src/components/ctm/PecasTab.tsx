...
export function PecasTab({ aircraftId }: PecasTabProps) {
  ...
  async function loadPecas() {
    // Busca ctm_pecas_trocadas que são ligadas via ordem_servico_id (OAS)
    const { data, error } = await supabase
      .from('ctm_pecas_trocadas')
      .select(`
        *,
        ctm_ras!inner(numero, aeronave_id, data_entrada, tipo_manutencao)
      `)
      .order('criado_em', { ascending: false });

    // Filtrar por aeronave via join
    let filteredData: any[] = [];
    if (data) {
      filteredData = data.filter((p: any) => p.ctm_ras?.aeronave_id === aircraftId);
    }

    // Também busca direto por pecas sem join (fallback)
    const { data: directData } = await supabase
      .from('ctm_pecas_trocadas')
      .select('*')
      .order('criado_em', { ascending: false });

    console.log('Pecas trocadas:', data, error, directData);
                  ...
                  return (
                    <tr key={p.id} className="border-b border-border/50 hover:bg-secondary/50 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        ...
                        {p.ordem_servico_id && !rasNum && (
                          <span className="badge-pending">OAS</span>
                        )}
                      </td>
...
function NovaPecaForm({ aircraftId, onClose, onSaved }: {
  ...
  async function save() {
    ...
    const { error } = await supabase.from('ctm_pecas_trocadas').insert({
      ordem_servico_id: form.ras_id || null,
    ...
    <div className="ctm-card p-5 mb-6 border-[hsl(var(--ctm-teal)/0.3)]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
        <div className="md:col-span-2 lg:col-span-3">
          <label className="text-xs text-muted-foreground block mb-1">Referência RAS (opcional)</label>
