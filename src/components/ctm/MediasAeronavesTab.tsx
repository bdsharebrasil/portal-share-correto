...
export function MediasAeronavesTab({ aircraftId, registration }: MediasAeronavesTabProps) {
  ...
  useEffect(() => {
    supabase
      ...
      .select('data_registro, horas_totais, tempo_total, pousos_total, consumo_combustivel_voo, clientes_id, natureza_voo')
      .eq('aeronave_id', aircraftId)
      .order('data_registro', { ascending: true })
      .then(({ data, error }) => {
        console.log('Medias:', data, error);
  ...
  entries.forEach(e => {
    ...
    porMes[key].horas += Number(e.horas_totais || e.tempo_total || 0);
