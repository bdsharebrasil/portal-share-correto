...
import { Plus, Edit2, Trash2, FileUp, DollarSign, Search, X, Upload, FileText, AlertCircle } from "lucide-react";
...
const getErrorMessage = (error: any): string => {
  ...
  // Verificar se é erro com código
  if (error?.code) {
    const errorMap: { [key: string]: string } = {
      '23502': 'Campo obrigatório não preenchido',
      '23503': 'Referência inválida (FK constraint)',
      '23505': 'Duplicata - este número já existe',
      '23514': 'Valor inválido (constraint violado)',
      '42501': 'Permissão negada',
      'PGRST116': 'Tabela não encontrada',
    };
    return errorMap[error.code] || `Erro ${error.code}: ${error.message}`;
  }
  ...
  // Fallback
  return 'Erro desconhecido ao salvar nota';
};
...
export function NotasFiscaisSaida() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.group("📝 Salvando Nota Fiscal");
      console.log("Modo:", editingNota ? "UPDATE" : "INSERT");
      console.log("Dados:", JSON.stringify(notaData, null, 2));
      console.groupEnd();

      if (editingNota) {
        console.log(`Atualizando nota ${editingNota.id}...`);
        ...
        if (updateError) {
          logError("UPDATE - notas_fiscais_saida", updateError);
          throw updateError;
        }

        console.log("Nota atualizada com sucesso:", updateData);
        ...
        console.log("Inserindo nova nota...");
        ...
        if (insertError) {
          logError("INSERT - notas_fiscais_saida", insertError);
          throw insertError;
        }

        console.log("Nota criada com sucesso:", insertData);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast({
        title: "Erro",
        description: `Erro ao deletar: ${errorMsg}`,
        variant: "destructive",
      });
    }
  };
  ...
  const handlePDFUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (uploadError) {
        logError("PDF Upload", uploadError);
        toast({
          title: "Erro",
      ...
      logError("handlePDFUpload", error);
      toast({
        title: "Erro",
  ...
  const handleChangeStatus = async (notaId: string, newStatus: string) => {
    ...
    try {
      ...
      toast({
        title: "Erro",
        description: `Erro ao atualizar status: ${errorMsg}`,
        variant: "destructive",
      });
    }
  };
  ...
  const handleGenerarRecibo = async () => {
    ...
    try {
      ...
      logError("handleGenerarRecibo", error);
        ...
        <Card className="bg-red-500/10 border-red-500/30">
          <CardContent className="p-4 flex gap-3">
            ...
            <div className="flex-1">
              <p className="font-semibold text-red-600">Erro Detalhado:</p>
              <p className="text-sm text-red-500 mt-1 font-mono">{errorDetails}</p>
              <p className="text-xs text-red-400 mt-2">Verifique o console (F12) para mais informações</p>
            </div>
          </CardContent>
        </Card>
      )}
