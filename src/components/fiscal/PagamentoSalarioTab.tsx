...
export function PagamentoSalarioTab() {
  ...
  const createCashFlowEntry = async (employeeId: string, employeeName: string, salaryAmount: number | null, paymentId: string, comprovanteUrl: string | null) => {
    ...
    try {
      ...
      if (row.id) {
        ...
        if (data && row.base_salary_holerite && row.base_salary_holerite > 0) {
          await createCashFlowEntry(row.user_profile, row.employee_name, row.base_salary_holerite, data.id, row.comprovante_url);
        }
      }
  ...
  return <div className="h-full flex flex-col gap-6">
      {/* Header */}
      <div className="flex-shrink-0 my-0 py-0 gap-[7px] flex items-center justify-between">
        <div>
        ...
        {!showPeriodSelector && <div className="flex items-center gap-2">
      ...
      {/* Period Selector */}
      {showPeriodSelector ? <Card className="border-primary/20 flex-shrink-0">
          <CardHeader>
          ...
          <CardContent>
            <div className="flex flex-col sm:flex-row gap-4 items-end">
              ...
              <Button onClick={loadPaymentsForPeriod} disabled={isLoading || !selectedMonth || !selectedYear} className="bg-primary hover:bg-primary/90">
              ...
              <div className="overflow-x-auto overflow-y-auto flex-1">
                <div className="min-w-[1800px]">
                  ...
                  {paymentRows.map((row, index) => <div key={row.user_profile} className={`grid grid-cols-[160px_100px_90px_90px_90px_90px_90px_90px_120px_100px_80px_90px_90px_60px_50px] gap-2 p-3 border-b items-center hover:bg-muted/30 transition-colors ${row.isDirty ? "bg-yellow-500/10" : ""}`}>
                      ...
                      {/* Salary */}
                      <Input type="number" step="0.01" min="0" placeholder="0.00" value={row.base_salary_holerite || ""} onChange={e => updateRow(index, "base_salary_holerite", e.target.value ? parseFloat(e.target.value) : null)} className="h-8 text-xs" />

                      {/* Benefit */}
                      <Input type="text" placeholder="Benef." value={row.benefit} onChange={e => updateRow(index, "benefit", e.target.value)} className="h-8 text-xs" />

                      {/* Flight Hours */}
                      <Input type="text" placeholder="Horas" value={row.horas_voo} onChange={e => updateRow(index, "horas_voo", e.target.value)} className="h-8 text-xs" />

                      {/* 13th Salary - Installment 1 */}
                      <Input type="number" step="0.01" min="0" placeholder="1ª Parc." value={row.decimo_terceiro_parcela1 || ""} onChange={e => updateRow(index, "decimo_terceiro_parcela1", e.target.value ? parseFloat(e.target.value) : null)} className="h-8 text-xs" title="1ª Parcela do 13º Salário" />

                      {/* 13th Salary - Installment 2 */}
                      <Input type="number" step="0.01" min="0" placeholder="2ª Parc." value={row.decimo_terceiro_parcela2 || ""} onChange={e => updateRow(index, "decimo_terceiro_parcela2", e.target.value ? parseFloat(e.target.value) : null)} className="h-8 text-xs" title="2ª Parcela do 13º Salário" />

                      {/* Vacation */}
                      <Input type="number" step="0.01" min="0" placeholder="Férias" value={row.ferias || ""} onChange={e => updateRow(index, "ferias", e.target.value ? parseFloat(e.target.value) : null)} className="h-8 text-xs" title="Pagamento de Férias" />

                      {/* Extra */}
                      <Input type="text" placeholder="Extra" value={row.extra} onChange={e => updateRow(index, "extra", e.target.value)} className="h-8 text-xs" />

                      {/* Observations */}
                      <Input type="text" placeholder="Obs" value={row.obs} onChange={e => updateRow(index, "obs", e.target.value)} className="h-8 text-xs" />
                      ...
                      <Select value={row.banco} onValueChange={value => updateRow(index, "banco", value)}>
                        ...
                        <SelectContent align="start" className="text-xs">
                          {bancos.map(banco => <SelectItem key={banco} value={banco}>
                              {banco}
                            </SelectItem>)}
                        </SelectContent>
                      </Select>
                      ...
                      if (file) handleFileUpload(index, file, "holerite");
                        ...
                        {row.holerite_url && <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => openPreview(row.holerite_url!, "holerite")} title="Visualizar Holerite">
                      ...
                      if (file) handleFileUpload(index, file, "comprovante");
                        ...
                        {row.comprovante_url && <Button variant="ghost" size="sm" className="h-7 w-7 p-0" onClick={() => openPreview(row.comprovante_url!, "comprovante")} title="Visualizar Comprovante">
                            <Eye className="h-4 w-4 text-green-500" />
                          </Button>}
                      </div>
                  ...
                  {paymentRows.length === 0 && <div className="p-8 text-center text-muted-foreground">
        ...
        <DialogContent className="max-w-4xl max-h-[90vh]">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              {previewType === "holerite" ? <FileText className="h-5 w-5" /> : <Receipt className="h-5 w-5" />}
              {previewType === "holerite" ? "Holerite" : "Comprovante de Pagamento"}
            </DialogTitle>
          </DialogHeader>
          <div className="flex-1 overflow-auto">
            {previewUrl && (previewUrl.toLowerCase().endsWith(".pdf") ? <iframe src={previewUrl} className="w-full h-[70vh] border rounded-lg" title="Preview" /> : <img src={previewUrl} alt="Preview" className="max-w-full h-auto mx-auto rounded-lg" />)}
          </div>
        </DialogContent>
      </Dialog>
    </div>;
