    ...
    <div style={{ pageBreakBefore: "always", breakBefore: "page" }} className="mb-8">
      ...
      <div className="mb-6">
        ...
        {travelReports.length === 0 ? (
          <p className="text-sm text-gray-400 italic">Nenhum relatório de viagem registrado.</p>
        ) : (
          <table className="w-full text-[10px] border-collapse">
            <thead>
              <tr className="bg-[#f8fafc]">
                {["Nº Relatório", "Período", "Rota", "Dias", "Combustível", "Hospedagem", "Alimentação", "Transporte", "Total", "Status"].map(h => (
                  <th key={h} className="px-2 py-2 text-left font-semibold text-[#1a1a2e] uppercase border-b-2 border-[#e2e8f0]">{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {travelReports.map((r, i) => (
                <tr key={r.id} className={i % 2 === 0 ? "bg-white" : "bg-[#f8fafc]"}>
                  <td className="px-2 py-1.5 border-b border-[#e2e8f0] font-medium">{r.report_number}</td>
                  <td className="px-2 py-1.5 border-b border-[#e2e8f0]">{fmtDate(r.start_date)} - {fmtDate(r.end_date)}</td>
                  <td className="px-2 py-1.5 border-b border-[#e2e8f0]">{r.route || "—"}</td>
                  <td className="px-2 py-1.5 border-b border-[#e2e8f0]">{r.days_count}</td>
                  <td className="px-2 py-1.5 border-b border-[#e2e8f0]">{fmt(r.total_fuel || 0)}</td>
                  <td className="px-2 py-1.5 border-b border-[#e2e8f0]">{fmt(r.total_lodging || 0)}</td>
                  <td className="px-2 py-1.5 border-b border-[#e2e8f0]">{fmt(r.total_food || 0)}</td>
                  <td className="px-2 py-1.5 border-b border-[#e2e8f0]">{fmt(r.total_transport || 0)}</td>
                  <td className="px-2 py-1.5 border-b border-[#e2e8f0] font-medium">{fmt(r.total_amount || 0)}</td>
                  <td className="px-2 py-1.5 border-b border-[#e2e8f0]">
                    <span className={`inline-block px-2 py-0.5 rounded-full text-[9px] font-bold ${
                      (r.status === "pago" || r.status === "paid") ? "bg-[#10b981]/20 text-[#10b981]" :
                      r.status === "finalizado" ? "bg-[#3b82f6]/20 text-[#3b82f6]" :
                      "bg-[#f59e0b]/20 text-[#f59e0b]"
                    }`}>
                      {STATUS_LABELS[r.status || ""] || r.status || "rascunho"}
                    </span>
                  </td>
                </tr>
              ))}
            ...
            <tfoot>
              <tr className="bg-[#1a1a2e] text-white font-bold">
                <td className="px-2 py-2" colSpan={4}>TOTAL VIAGENS</td>
                <td className="px-2 py-2">{fmt(travelReports.reduce((s, r) => s + (r.total_fuel || 0), 0))}</td>
                <td className="px-2 py-2">{fmt(travelReports.reduce((s, r) => s + (r.total_lodging || 0), 0))}</td>
                <td className="px-2 py-2">{fmt(travelReports.reduce((s, r) => s + (r.total_food || 0), 0))}</td>
                <td className="px-2 py-2">{fmt(travelReports.reduce((s, r) => s + (r.total_transport || 0), 0))}</td>
                <td className="px-2 py-2">{fmt(totalTravelR)}</td>
                <td className="px-2 py-2" />
              </tr>
            </tfoot>
          </table>
