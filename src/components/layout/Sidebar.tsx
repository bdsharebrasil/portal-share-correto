  ...
  const renderMenuContent = () => (
    <nav className="space-y-4">
      {filteredMenuGroups.map((group) => (
        <Card key={group.title} className="bg-card-secondary/40 border-border/50 shadow-none">
          ...
          <CardContent className="pt-0 space-y-2">
            {group.items.map((item) => (
              <div key={item.title}>
                {item.isExternal ? (
                  <a href={item.externalUrl} target="_blank" rel="noopener noreferrer" onClick={handleItemClick} className="flex items-center px-3 py-2 rounded-md text-sm font-medium border border-border/50 hover:bg-card-secondary/70 hover:border-primary/50 transition-smooth text-foreground">
                  ...
                  <Collapsible open={expandedItems.includes(item.title)} onOpenChange={() => toggleExpanded(item.title)}>
                    <CollapsibleTrigger asChild>
                      <Button variant="outline" className="w-full justify-between border-border/50 hover:bg-card-secondary/70 hover:border-primary/50 transition-smooth rounded-md">
                    ...
                    <CollapsibleContent className="ml-4 mt-1 space-y-1">
                      {item.subItems?.map((sub) => (
                        <NavLink key={sub.href} to={sub.href} className={({ isActive }) => cn("block px-4 py-2 text-sm rounded-md border transition-smooth", isActive ? "bg-primary/40 text-white shadow-primary border-primary/50" : "text-foreground border-border/50 hover:bg-card-secondary/70 hover:border-primary/50")} onClick={handleItemClick}>
                          {sub.title}
                        </NavLink>
                      ))}
                    </CollapsibleContent>
                  ...
                  <NavLink to={item.href || "#"} className={({ isActive }) => cn("flex items-center px-3 py-2 rounded-md text-sm font-medium border transition-smooth", isActive ? "bg-primary/40 text-white shadow-primary border-primary/50" : "text-foreground border-border/50 hover:bg-card-secondary/70 hover:border-primary/50")} onClick={handleItemClick}>
                    {item.icon && <item.icon className="mr-3 h-4 w-4 text-primary" />}
                    {item.title}
                  </NavLink>
                )}
