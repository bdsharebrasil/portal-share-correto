  ...
  return <aside className={cn("fixed right-0 top-16 h-[calc(100vh-4rem)] aviation-gradient-card border-l border-border transition-all duration-300 custom-scrollbar overflow-y-auto z-40", isOpen ? "w-80" : "w-0")}>
      {isOpen && <div className="p-4 space-y-6">
          ...
          <Card className="bg-gradient-card border-border shadow-card static-card rounded-lg">
            ...
            <CardContent className="pt-0 space-y-2">
              {maintenanceItems.map(item => <Button key={item.label} variant="outline" className="w-full justify-start border-border hover:bg-accent hover:border-primary transition-smooth rounded-md" onClick={() => navigate(item.path)}>
