...
export function ReceiptForm({
  ...
  // Fetch partners when client is selected
  ...
  const handleClienteSearchChange = (searchValue: typeof clienteSearchValue) => {
    ...
    // Fetch partners if a client was selected
    if (searchValue.clienteId) {
      fetchClientPartners(searchValue.clienteId);
    }
  };

  // Handle partner selection - update payer name/document to partner's
  const handleSelectPartner = (partnerName: string) => {
    ...
    if (partner) {
      setFormData((prev) => ({
        ...
        pagadorDocumento: partner.cpf || prev.pagadorDocumento,
      }));
    }
  };
      ...
      <Card>
        <CardContent className="p-6 space-y-6">
          ...
          <PartnerSelectModal
            ...
            clientName={clienteSearchValue.nome || formData.pagadorNome}
