...
export function ClienteSearchInput({
  ...
  const handleManualChange = (field: string, val: string) => {
    onChange({
      ...
      [field]: val,
      clienteId: "",
      // Limpa o ID quando entramos em modo manual
        ...
        <TabsContent value="search" className="space-y-3 mt-4">
          <div>
            ...
            <div className="relative">
              <Input placeholder="Digite o nome ou CNPJ do cliente..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} disabled={disabled} className="w-full" />
          ...
          {showSuggestions && <div className="border border-border rounded-lg p-3 bg-muted/30 max-h-60 overflow-y-auto space-y-2">
        ...
        <TabsContent value="manual" className="space-y-4 mt-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Input value={value.nome} onChange={e => handleManualChange("nome", e.target.value)} placeholder="Razão social ou nome da empresa" disabled={disabled} required={required} />
            </div>
          ...
          <div>
            ...
            <Input value={value.endereco} onChange={e => handleManualChange("endereco", e.target.value)} placeholder="Rua, número, complemento" disabled={disabled} />
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <Label>Cidade</Label>
              <Input value={value.cidade} onChange={e => handleManualChange("cidade", e.target.value)} placeholder="São Paulo" disabled={disabled} />
            </div>

            <div>
              <Label>UF</Label>
              <Input value={value.uf} onChange={e => handleManualChange("uf", e.target.value.toUpperCase().slice(0, 2))} placeholder="SP" disabled={disabled} maxLength={2} />
            </div>
          </div>
