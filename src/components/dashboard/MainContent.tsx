        ...
        <Card className="bg-gradient-card border-border shadow-card rounded-lg">
          <CardHeader className="flex flex-row items-center justify-between border-b border-border/50 pb-4">
            <CardTitle className="flex items-center text-foreground gap-2">
              <Clock className="h-5 w-5 text-custom-cyan" />
              Tarefas do Dia
            </CardTitle>
            <Badge className="bg-custom-cyan text-background font-semibold px-4 py-1">
            ...
            {todayTasks.length === 0 ? (
              ...
              <div className="space-y-3">
                {todayTasks.map((task) => (
                  <div
                    key={task.id}
                    className="flex items-center gap-3 py-2 px-2 hover:bg-muted/30 rounded-md transition-colors cursor-pointer group"
                    onClick={() => toggleTask(task, task.status !== 'concluida')}
                  >
                    <div className="flex-shrink-0 flex items-center justify-center">
                      <div
                        className={`h-5 w-5 rounded-full border-2 flex items-center justify-center transition-all ${
                          task.status === 'concluida'
                            ? 'border-custom-cyan bg-custom-cyan'
                            : 'border-muted-foreground/40 group-hover:border-custom-cyan'
                        }`}
                      >
                        {task.status === 'concluida' && (
