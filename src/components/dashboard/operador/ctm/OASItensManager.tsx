...
export function OASItensManager({ oasId, aircraftId, onChanged }: { oasId: string; aircraftId: string; onChanged?: () => void }) {
  ...
  return <div className="space-y-5">
    <div className="ctm-card overflow-hidden border-cyan-400/15 bg-gradient-to-br from-card/80 via-slate-950/70 to-cyan-950/10">
      <div className="flex flex-col gap-4 border-b border-white/10 p-5 sm:flex-row sm:items-center sm:justify-between">
        <div><div className="mb-1 flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-cyan-300"><Receipt className="h-4 w-4" /> Histórico financeiro da OAS</div><h3 className="text-lg font-bold text-foreground">Serviços e peças realizados</h3><p className="mt-1 text-xs text-muted-foreground">Serviços e peças desta OAS.</p></div>
    ...
    <Dialog open={Boolean(preview)} onOpenChange={(open) => !open && setPreview(null)}><DialogContent className="flex h-[90vh] w-[96vw] max-w-5xl flex-col overflow-hidden border-cyan-300/20 bg-background p-0"><DialogHeader className="flex shrink-0 flex-row items-center justify-between border-b border-white/10 px-5 py-4"><DialogTitle className="flex min-w-0 items-center gap-2 truncate text-sm text-foreground"><FileText className="h-4 w-4 text-cyan-300" /> {preview?.name || 'Nota fiscal'}</DialogTitle></DialogHeader>{preview && (preview.url.toLowerCase().includes('.pdf') ? <iframe title={preview.name} src={preview.url} className="min-h-0 flex-1 bg-white" /> : <div className="min-h-0 flex-1 overflow-auto bg-card p-4 text-center"><img src={preview.url} alt={preview.name} className="mx-auto max-h-full max-w-full object-contain" /></div>)}</DialogContent></Dialog>
  </div>;
}
...
function EditItemModal({ editing, form, setForm, rates, setRates, cotistas, saving, onClose, onSave }: any) {
  ...
  return <Dialog open onOpenChange={(open) => !open && onClose()}><DialogContent className="max-h-[92vh] w-[96vw] max-w-3xl overflow-y-auto border-cyan-300/20 bg-background"><DialogHeader><DialogTitle className="text-foreground">Editar {service ? 'serviço' : 'peça'}</DialogTitle></DialogHeader><div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><Field label={service ? 'Oficina' : 'Fornecedor'}><input className="ctm-input w-full" value={service ? form.oficina_nome : form.fornecedor} onChange={(e) => update(service ? 'oficina_nome' : 'fornecedor', e.target.value)} /></Field>{service && <Field label="Nº da OS"><input className="ctm-input w-full" value={form.os_oficina} onChange={(e) => update('os_oficina', e.target.value)} /></Field>}<Field label={service ? 'Valor' : 'Valor total'}><input className="ctm-input w-full" inputMode="decimal" value={service ? form.valor : form.valor_total} onChange={(e) => update(service ? 'valor' : 'valor_total', e.target.value)} /></Field>{!service && <Field label="Quantidade"><input className="ctm-input w-full" inputMode="decimal" value={form.quantidade} onChange={(e) => update('quantidade', e.target.value)} /></Field>}<Field label="Nº da nota fiscal"><input className="ctm-input w-full" value={form.numero_nota_fiscal} onChange={(e) => update('numero_nota_fiscal', e.target.value)} /></Field><Field label={service ? 'Data do serviço' : 'Data da compra'}><input type="date" className="ctm-input w-full" value={service ? form.data_servico : form.data_compra} onChange={(e) => update(service ? 'data_servico' : 'data_compra', e.target.value)} /></Field>{service && <Field label="Base do rateio"><select className="ctm-input w-full" value={form.tipo_rateio} onChange={(e) => update('tipo_rateio', e.target.value)}><option value="cota">Cota da aeronave</option><option value="uso">Percentual por uso</option></select></Field>}<div className="sm:col-span-2"><Field label={service ? 'Descrição do serviço' : 'Descrição da peça'}><textarea className="ctm-input min-h-24 w-full resize-y" value={form.descricao} onChange={(e) => update('descricao', e.target.value)} /></Field></div><div className="sm:col-span-2"><Field label="Substituir nota fiscal"><input className="ctm-input w-full file:mr-2 file:rounded-lg file:border-0 file:bg-cyan-400/15 file:px-2 file:py-1 file:text-xs file:text-cyan-100" type="file" accept="application/pdf,image/*" onChange={(e) => update('nota_fiscal_file', e.target.files?.[0] || null)} /></Field></div></div><div className="mt-4 rounded-xl border border-white/10 bg-white/[0.025] p-3"><div className="mb-2 flex items-center justify-between"><p className="text-xs font-semibold text-foreground">Rateio</p><span className={Math.abs(total - 100) < 0.01 ? 'text-xs text-emerald-300' : 'text-xs text-amber-200'}>{total.toFixed(2)}%</span></div><div className="grid grid-cols-1 gap-2 sm:grid-cols-3">{cotistas.map((cotista: any) => <label key={cotista.socio_id} className="flex items-center justify-between gap-2 rounded-lg border border-white/10 px-3 py-2"><span className="truncate text-xs text-foreground">{cotista.nome}</span><input className="ctm-input h-8 w-20 text-right" inputMode="decimal" value={rates[cotista.socio_id] || '0'} onChange={(e) => setRates((current: any) => ({ ...current, [cotista.socio_id]: e.target.value }))} /></label>)}</div></div><div className="mt-5 flex justify-end gap-2"><button onClick={onClose} className="rounded-xl px-4 py-2 text-sm text-muted-foreground hover:bg-white/5">Cancelar</button><button disabled={saving} onClick={onSave} className="inline-flex items-center gap-2 rounded-xl bg-cyan-300 px-4 py-2 text-sm font-semibold text-slate-950 disabled:opacity-50"><Save className="h-4 w-4" />Salvar alterações</button></div></DialogContent></Dialog>;
}

function Summary({ label, value, icon, strong = false }: { label: string; value: number; icon: React.ReactNode; strong?: boolean }) { return <div className={`bg-background/45 p-4 ${strong ? 'sm:bg-cyan-950/20' : ''}`}><div className="flex items-center gap-2 text-xs font-semibold text-muted-foreground">{icon}{label}</div><p className={`mt-2 text-xl font-bold ${strong ? 'text-cyan-200' : 'text-foreground'}`}>{money(value)}</p></div>; }
function Field({ label, children, wide = false }: { label: string; children: React.ReactNode; wide?: boolean }) { return <label className={wide ? 'sm:col-span-2 lg:col-span-2' : ''}><span className="mb-1 block text-xs font-semibold text-muted-foreground">{label}</span>{children}</label>; }
function ItemSection({ title, icon, empty, children }: { title: string; icon: React.ReactNode; empty: string; children: React.ReactNode }) { const hasItems = Array.isArray(children) ? children.length > 0 : Boolean(children); return <section className="ctm-card overflow-hidden"><div className="flex items-center justify-between border-b border-white/10 bg-white/[0.025] px-5 py-4"><h3 className="flex items-center gap-2 font-bold text-foreground">{icon}<span>{title}</span></h3></div><div className="space-y-3 p-4">{hasItems ? children : <p className="py-4 text-sm text-muted-foreground">{empty}</p>}</div></section>; }
function InvoiceCard({ group, onPreview, onEdit }: { group: { number: string; items: Array<{ id: string; kind: 'Serviço' | 'Peça'; description: string; supplier: string; period: string; value: number; rateios: Rateio[]; notaPath: string | null; notaName: string | null }> }; onPreview: (path: string, name: string) => void; onEdit: (item: { id: string; kind: 'Serviço' | 'Peça' }) => void }) { const total = group.items.reduce((sum, item) => sum + item.value, 0); const rateio = group.items.flatMap((item) => item.rateios).reduce<Record<string, { nome?: string; valor: number }>>((acc, row) => { const key = row.socio_id || row.cliente_id; const current = acc[key] || { nome: row.nome, valor: 0 }; current.valor += Number(row.valor || 0); acc[key] = current; return acc; }, {}); const attachment = group.items.find((item) => item.notaPath); return <article className="overflow-hidden rounded-2xl border border-cyan-300/15 bg-gradient-to-br from-cyan-950/25 via-slate-950/55 to-background/70 shadow-lg shadow-cyan-950/10"><header className="flex flex-col gap-3 border-b border-white/10 bg-white/[0.035] px-4 py-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><span className="rounded-xl border border-cyan-300/20 bg-cyan-400/10 p-2.5 text-cyan-200"><Receipt className="h-5 w-5" /></span><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-cyan-200/75">Nota fiscal</p><h4 className="text-lg font-bold text-foreground">{group.number}</h4><p className="text-xs text-muted-foreground">{group.items.length} {group.items.length === 1 ? 'item vinculado' : 'itens vinculados'}</p></div></div><div className="flex items-center gap-3"><div className="text-left sm:text-right"><p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Total da nota</p><p className="text-xl font-bold text-cyan-200">{money(total)}</p></div>{attachment && <button type="button" onClick={() => onPreview(attachment.notaPath!, attachment.notaName || group.number)} className="inline-flex items-center gap-1.5 rounded-xl border border-cyan-300/25 bg-cyan-400/10 px-3 py-2 text-xs font-bold text-cyan-100 transition hover:bg-cyan-400/20"><Eye className="h-4 w-4" /> Ver nota</button>}</div></header><div className="overflow-x-auto"><table className="min-w-[680px] w-full text-left text-xs"><thead className="border-b border-white/10 bg-background/30 text-[10px] uppercase tracking-wider text-muted-foreground"><tr><th className="px-4 py-3">Item</th><th className="px-4 py-3">Fornecedor</th><th className="px-4 py-3">Período</th><th className="px-4 py-3 text-right">Valor</th><th className="px-4 py-3 text-right">Ação</th></tr></thead><tbody className="divide-y divide-white/10">{group.items.map((item) => <tr key={item.id} className="transition hover:bg-cyan-400/[0.04]"><td className="max-w-[460px] px-4 py-3 font-medium text-foreground">{item.description}<span className="ml-2 rounded-full border border-white/10 px-1.5 py-0.5 text-[9px] uppercase text-muted-foreground">{item.kind}</span></td><td className="px-4 py-3 text-muted-foreground">{item.supplier}</td><td className="px-4 py-3 text-muted-foreground">{item.period}</td><td className="px-4 py-3 text-right font-semibold text-foreground">{money(item.value)}</td><td className="px-4 py-3 text-right"><button type="button" onClick={() => onEdit(item)} className="inline-flex items-center gap-1 rounded-lg border border-cyan-300/20 px-2 py-1.5 text-[10px] font-semibold text-cyan-200 hover:bg-cyan-400/10"><Pencil className="h-3 w-3" /> Editar</button></td></tr>)}</tbody></table></div>{Object.keys(rateio).length > 0 && <footer className="border-t border-white/10 bg-background/25 px-4 py-3"><div className="mb-2 flex items-center justify-between"><p className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground">Rateio consolidado desta nota</p><span className="text-xs font-semibold text-cyan-200">{money(total)}</span></div><div className="grid grid-cols-1 gap-2 sm:grid-cols-3">{Object.entries(rateio).map(([id, row]) => <div key={id} className="flex items-center justify-between rounded-lg border border-white/10 bg-white/[0.035] px-3 py-2 text-xs"><span className="truncate pr-2 text-muted-foreground">{row.nome || `Cotista ${id.slice(0, 6)}`}</span><span className="font-semibold text-cyan-200">{money(row.valor)}</span></div>)}</div></footer>}</article>; }
