...
import { FileText, Plus, Receipt, Save, Trash2, Wrench, X, Paperclip, Eye } from 'lucide-react';
...
function ItemCard({ title, meta, value, rateios, notaPath, notaName, onPreview }: { title: string; meta: string; value: number; rateios: Rateio[]; notaPath?: string | null; notaName?: string | null; onPreview: (path: string, name: string) => void }) { return <article className="rounded-2xl border border-white/10 bg-slate-950/45 p-4 transition hover:border-cyan-300/20 hover:bg-cyan-950/10"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><h4 className="font-semibold text-slate-100">{title}</h4><p className="mt-1 text-xs text-slate-400">{meta || 'Sem informações complementares'}</p></div><div className="flex shrink-0 items-center gap-3"><p className="text-base font-bold text-cyan-200">{money(value)}</p>{notaPath && <button type="button" onClick={() => onPreview(notaPath, notaName || 'Nota fiscal')} className="inline-flex items-center gap-1.5 rounded-lg border border-cyan-300/25 bg-cyan-400/10 px-2.5 py-1.5 text-[11px] font-bold text-cyan-100 transition hover:bg-cyan-400/20"><Eye className="h-3.5 w-3.5" /> Ver NF</button>}</div></div>{rateios.length > 0 && <div className="mt-4 border-t border-white/10 pt-3"><p className="mb-2 text-[10px] font-bold uppercase tracking-wider text-slate-400">Rateio deste lançamento</p><div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-3">{rateios.map((row) => <div key={row.id} className="flex items-center justify-between rounded-lg bg-white/[0.035] px-3 py-2 text-xs"><span className="truncate pr-2 text-slate-300">{row.nome || `Cotista ${row.cliente_id.slice(0, 6)}`}</span><span className="font-semibold text-cyan-200">{Number(row.percentual || 0).toFixed(2)}% · {money(row.valor)}</span></div>)}</div></div>}</article>; }
