...
export function RastreamentoTab({ aircraftId }: { aircraftId: string }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-col gap-4 rounded-3xl border border-cyan-300/15 bg-gradient-to-br from-cyan-400/[0.08] via-slate-950/70 to-background/80 p-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm leading-6 text-muted-foreground">Consulte quando cada componente foi instalado, inspecionado ou removido, em qual OAS o evento ocorreu e quais documentos comprovam o serviço.</p>
        </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(280px,0.8fr)_minmax(0,1.6fr)]">
        <section className="rounded-2xl border border-white/10 bg-background/65 p-4">
          <div className="mb-3 flex items-center justify-between"><div><p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Inventário</p><h3 className="mt-1 font-semibold text-white">Componentes acompanhados</h3></div><span className="rounded-full border border-cyan-300/20 bg-cyan-300/10 px-2 py-1 text-xs text-cyan-200">{components.length}</span></div>
          <label className="relative mb-3 block"><Search className="absolute left-3 top-2.5 h-4 w-4 text-muted-foreground" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar nome, P/N, S/N ou posição" className="w-full rounded-xl border border-white/10 bg-white/[0.04] py-2 pl-9 pr-3 text-sm text-white outline-none placeholder:text-muted-foreground focus:border-cyan-300/40" /></label>
          <div className="max-h-[560px] space-y-2 overflow-y-auto pr-1">
            {filtered.map((item) => {
              const itemEvents = events.filter((event) => event.componente_id === item.id);
              const active = item.id === selectedId;
              return <button key={item.id} onClick={() => setSelectedId(item.id)} className={cn('w-full rounded-xl border p-3 text-left transition', active ? 'border-cyan-300/35 bg-cyan-300/[0.09]' : 'border-white/10 bg-white/[0.025] hover:border-white/20 hover:bg-white/[0.05]')}><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-sm font-semibold text-white">{item.nome || 'Componente sem nome'}</p><p className="mt-1 truncate text-xs text-muted-foreground">{[item.localizacao, item.categoria].filter(Boolean).join(' · ') || 'Posição não informada'}</p></div><span className={cn('mt-0.5 h-2 w-2 shrink-0 rounded-full', statusDot(item.status))} /></div><div className="mt-2 flex items-center justify-between text-[11px] text-muted-foreground"><span>{item.numero_de_serie ? `S/N ${item.numero_de_serie}` : item.numero_da_peça ? `P/N ${item.numero_da_peça}` : 'Sem identificação'}</span><span>{itemEvents.length} evento(s)</span></div></button>;
            })}
            {filtered.length === 0 && <div className="rounded-xl border border-dashed border-white/10 p-6 text-center text-sm text-muted-foreground"><PackageSearch className="mx-auto mb-2 h-6 w-6" />Nenhum componente encontrado.</div>}
          </div>
        </section>

        <section className="rounded-2xl border border-white/10 bg-background/65 p-4 sm:p-5">
          {!selected ? <div className="flex min-h-[360px] flex-col items-center justify-center text-center text-muted-foreground"><PackageSearch className="mb-3 h-10 w-10" /><p className="text-sm">Cadastre um componente no Mapa de Componente para iniciar o rastreamento.</p></div> : <>
            <div className="flex flex-col gap-4 border-b border-white/10 pb-4 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><div className="flex items-center gap-2"><h3 className="truncate text-lg font-semibold text-white">{selected.nome}</h3><span className={cn('rounded-full border px-2 py-0.5 text-[10px] font-semibold uppercase', statusBadge(status))}>{status || 'ativo'}</span></div><div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground"><span>P/N: <b className="text-foreground">{selected.numero_da_peça || '—'}</b></span><span>S/N: <b className="text-foreground">{selected.numero_de_serie || '—'}</b></span><span>Posição: <b className="text-foreground">{selected.localizacao || '—'}</b></span></div></div><button onClick={() => openNewEvent(selected.id)} className="inline-flex shrink-0 items-center justify-center gap-2 rounded-lg border border-cyan-300/25 bg-cyan-300/10 px-3 py-2 text-xs font-semibold text-cyan-100 hover:bg-cyan-300/15"><Plus className="h-3.5 w-3.5" /> Novo evento</button></div>
            ...
            <div className="mt-5"><div className="mb-3 flex items-center justify-between"><div><p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Linha do tempo</p><h4 className="mt-1 font-semibold text-white">Histórico documentado</h4></div><span className="text-xs text-muted-foreground">{selectedEvents.length} registro(s)</span></div>{selectedEvents.length === 0 ? <div className="rounded-xl border border-dashed border-white/10 p-8 text-center text-sm text-muted-foreground">Nenhum evento técnico registrado para este componente.</div> : <div className="space-y-3">{selectedEvents.map((event) => <EventCard key={event.id} event={event} oas={oas} />)}</div>}</div>
          </>}
        </section>
      </div>
...
function Metric({ icon: Icon, label, value }: { icon: typeof Activity; label: string; value: string }) { return <div className="rounded-xl border border-white/10 bg-white/[0.025] p-3"><div className="flex items-center gap-2 text-[10px] uppercase tracking-wider text-muted-foreground"><Icon className="h-3.5 w-3.5 text-cyan-300" />{label}</div><p className="mt-2 text-sm font-semibold text-white">{value}</p></div>; }
...
function EventCard({ event, oas }: { event: Component; oas: Component[] }) {
  ...
  return <div className="relative rounded-xl border border-white/10 bg-white/[0.025] p-4"><div className="flex gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-cyan-300/20 bg-cyan-300/10 text-cyan-200"><Icon className="h-4 w-4" /></div><div className="min-w-0 flex-1"><div className="flex flex-wrap items-center justify-between gap-2"><div><span className="text-sm font-semibold text-white">{EVENT_LABELS[type] || event.tipo_evento}</span><span className="ml-2 text-xs text-muted-foreground">{dateBR(event.data_evento)}</span></div>{linkedOas && <span className="rounded-full border border-blue-300/20 bg-blue-300/10 px-2 py-1 text-[10px] font-medium text-blue-200">OAS #{linkedOas.numero || '—'}</span>}</div><p className="mt-2 text-sm leading-5 text-muted-foreground">{event.descricao}</p><div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-muted-foreground"><span>Horas: <b className="text-muted-foreground">{num(event.horas_aeronave)}</b></span><span>Pousos: <b className="text-muted-foreground">{num(event.pousos_aeronave)}</b></span>{event.posicao && <span>Posição: <b className="text-muted-foreground">{event.posicao}</b></span>}{event.motivo && <span>Motivo: <b className="text-muted-foreground">{event.motivo}</b></span>}</div>{(event.s_n_removido || event.s_n_instalado || event.documento_url) && <div className="mt-3 flex flex-wrap items-center gap-2 border-t border-white/10 pt-3 text-xs">{event.s_n_removido && <span className="text-rose-300">S/N removido: {event.s_n_removido}</span>}{event.s_n_instalado && <span className="text-emerald-300">S/N instalado: {event.s_n_instalado}</span>}{event.documento_url && <a href={event.documento_url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-cyan-200 hover:text-white"><FileText className="h-3.5 w-3.5" />{event.documento_nome || 'Documento'}</a>}</div>}</div></div></div>;
}
