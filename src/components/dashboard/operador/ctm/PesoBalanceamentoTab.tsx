...
function TechnicalSheet({ wb, stations, limits, setLimits, savingLimits, saveLimits, showStationForm, setShowStationForm, stationForm, setStationForm, saveStation, updateStation, deleteStation }: any) {
  ...
  return <>
    ...
    <div className="grid grid-cols-1 gap-5 xl:grid-cols-[1.15fr_1fr]">
      <div className="rounded-2xl border border-slate-800/70 bg-slate-950/60 p-5">
        <div className="mb-4 flex items-center justify-between gap-3"><div><h3 className="text-sm font-semibold text-slate-200">Estrutura da ficha técnica</h3><p className="text-xs text-slate-500">Pesos e braços de referência mantidos pela oficina.</p></div><button onClick={() => setShowStationForm((value: boolean) => !value)} className="inline-flex items-center gap-1.5 whitespace-nowrap rounded-full border border-cyan-400/35 bg-transparent px-4 py-1.5 text-sm font-medium text-cyan-300 transition-colors hover:bg-cyan-400/10 print:hidden"><Plus className="h-3.5 w-3.5" /> Nova estação</button></div>
...
function LoadingsView({ loadings, showNewLoading, openNewLoading, setShowNewLoading, loadingForm, setLoadingForm, loadingItems, updateLoadingItem, loadingCalc, justification, setJustification, finalizeLoading, savingLoading }: any) {
  return <div className="space-y-5"><div className="flex flex-col gap-3 rounded-2xl border border-slate-800/70 bg-slate-950/60 p-5 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="text-lg font-bold text-slate-100">Carregamentos dos voos</h3><p className="text-xs text-slate-500">A ficha técnica é copiada como snapshot. O piloto informa somente os pesos do voo.</p></div><button onClick={openNewLoading} className="inline-flex items-center gap-1.5 whitespace-nowrap rounded-full border border-cyan-400/35 bg-transparent px-4 py-1.5 text-sm font-medium text-cyan-300 transition-colors hover:bg-cyan-400/10"><Plus className="h-3.5 w-3.5" /> Novo Carregamento</button></div>
