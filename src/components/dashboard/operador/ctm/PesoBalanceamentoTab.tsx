...
function AircraftCGIndicator({ calc }: { calc: any }) {
  ...
  const bodyColor = hasError ? '#f87171' : hasWarn ? '#fbbf24' : '#38bdf8';
  ...
  return (
    <div className="rounded-2xl border border-border/70 bg-background/60 p-5">
      <div className="mb-3 flex items-center justify-between gap-3"><h3 className="text-sm font-semibold text-foreground">Situação do carregamento</h3><span className="rounded-full border border-border bg-card/70 px-2 py-1 text-[10px] uppercase tracking-wider text-muted-foreground">CG em tempo real</span></div>
      ...
      {hasEnvelope && (
        <div className="mb-2">
          <div className="relative h-3 overflow-visible rounded-full bg-card-secondary">
...
export function PesoBalanceamentoTab({ aircraftId }: { aircraftId: string }) {
  ...
  if (loading) return <div className="flex items-center justify-center rounded-2xl border border-border/70 bg-background/50 p-16"><Loader2 className="h-6 w-6 animate-spin text-ctm-teal" /></div>;

  return (
    <div className="space-y-5 print:space-y-3">
      <div className="flex flex-col gap-4 rounded-2xl border border-border/70 bg-background/60 p-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div><h2 className="text-lg font-bold text-foreground">Peso &amp; Balanceamento</h2><p className="text-xs text-muted-foreground">{[aeronave?.matricula, [aeronave?.fabricante, aeronave?.modelo].filter(Boolean).join(' ')].filter(Boolean).join(' · ') || '—'}</p></div>
        </div>
        <button onClick={() => window.print()} className="inline-flex items-center gap-1.5 self-start rounded-lg border border-border px-3 py-1.5 text-xs text-muted-foreground transition-colors hover:bg-card-secondary print:hidden"><Printer className="h-3.5 w-3.5" /> Imprimir</button>
      </div>
      ...
      <div className="flex gap-2 border-b border-border print:hidden">
        ...
        <TabButton active={activeTab === 'carregamentos'} onClick={() => setActiveTab('carregamentos')}>Carregamentos <span className="ml-1 rounded-full bg-card-secondary px-1.5 py-0.5 text-[10px]">{loadings.length}</span></TabButton>
      </div>
...
function TabButton({ active, onClick, children }: { active: boolean; onClick: () => void; children: ReactNode }) {
  return <button onClick={onClick} className={cn('border-b-2 px-3 py-3 text-sm font-semibold transition-colors', active ? 'border-ctm-teal text-ctm-teal' : 'border-transparent text-muted-foreground hover:text-foreground')}>{children}</button>;
}
...
function TechnicalSheet({ wb, stations, limits, setLimits, savingLimits, saveLimits, showStationForm, setShowStationForm, stationForm, setStationForm, saveStation, updateStation, deleteStation }: any) {
  ...
  return <>
    ...
    <div className="grid grid-cols-1 gap-5 xl:grid-cols-[1.15fr_1fr]">
      <div className="rounded-2xl border border-border/70 bg-background/60 p-5">
        <div className="mb-4 flex items-center justify-between gap-3"><div><h3 className="text-sm font-semibold text-foreground">Estrutura da ficha técnica</h3><p className="text-xs text-muted-foreground">Pesos e braços de referência mantidos pela oficina.</p></div><button onClick={() => setShowStationForm((value: boolean) => !value)} className="inline-flex items-center gap-1.5 whitespace-nowrap rounded-full border border-cyan-400/35 bg-transparent px-4 py-1.5 text-sm font-medium text-cyan-300 transition-colors hover:bg-cyan-400/10 print:hidden"><Plus className="h-3.5 w-3.5" /> Nova estação</button></div>
        {showStationForm && <div className="mb-4 rounded-xl border border-border bg-card/50 p-4 print:hidden"><div className="mb-3 flex items-center justify-between"><p className="text-xs font-semibold text-muted-foreground">Nova estação</p><button onClick={() => setShowStationForm(false)}><X className="h-4 w-4 text-muted-foreground" /></button></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><input className="ctm-input" placeholder="Descrição (ex.: PILOTO)" value={stationForm.descricao} onChange={(e) => setStationForm((form: any) => ({ ...form, descricao: e.target.value }))} /><select className="ctm-input" value={stationForm.categoria} onChange={(e) => setStationForm((form: any) => ({ ...form, categoria: e.target.value }))}>{CATEGORIAS.map((category) => <option key={category} value={category}>{category}</option>)}</select><input className="ctm-input" placeholder="Peso de referência (kg)" inputMode="decimal" value={stationForm.peso} onChange={(e) => setStationForm((form: any) => ({ ...form, peso: e.target.value }))} /><input className="ctm-input" placeholder="Braço (m)" inputMode="decimal" value={stationForm.braco} onChange={(e) => setStationForm((form: any) => ({ ...form, braco: e.target.value }))} /></div><div className="mt-3 flex justify-end"><button onClick={saveStation} className="inline-flex items-center gap-1.5 rounded-lg bg-ctm-teal px-3 py-1.5 text-xs font-semibold text-[hsl(var(--ctm-navy))]"><Save className="h-3.5 w-3.5" /> Salvar estação</button></div></div>}
        <div className="overflow-x-auto"><table className="w-full text-xs"><thead><tr className="border-b border-border text-[11px] uppercase tracking-wide text-muted-foreground"><th className="px-2 py-2 text-left">Estação</th><th className="px-2 py-2 text-right">Peso (kg)</th><th className="px-2 py-2 text-right">Braço (m)</th><th className="px-2 py-2 text-right">Momento</th><th className="px-2 py-2 print:hidden" /></tr></thead><tbody>{stations.map((station: any) => <tr key={station.id} className="border-b border-slate-900/70 odd:bg-card/20"><td className="px-2 py-2"><p className="font-medium text-foreground">{station.descricao}</p><p className="text-[10px] uppercase text-muted-foreground">{station.categoria || 'OUTRO'}</p></td><td className="px-2 py-1.5 text-right"><input className="w-20 rounded border border-transparent bg-transparent px-1 py-0.5 text-right text-foreground outline-none hover:border-border focus:border-ctm-teal" defaultValue={station.peso_sem_combustivel ?? 0} onBlur={(e) => updateStation(station, { peso_sem_combustivel: n(e.target.value) })} /></td><td className="px-2 py-1.5 text-right"><input className="w-20 rounded border border-transparent bg-transparent px-1 py-0.5 text-right text-foreground outline-none hover:border-border focus:border-ctm-teal" defaultValue={station['braço_posicao'] ?? 0} onBlur={(e) => updateStation(station, { 'braço_posicao': n(e.target.value) })} /></td><td className="px-2 py-2 text-right text-muted-foreground">{fmt(n(station.peso_sem_combustivel) * n(station['braço_posicao']), 1)}</td><td className="px-2 py-2 text-right print:hidden"><button onClick={() => deleteStation(station.id)} className="text-muted-foreground hover:text-red-400"><Trash2 className="h-3.5 w-3.5" /></button></td></tr>)}{stations.length === 0 && <tr><td colSpan={5} className="py-8 text-center text-muted-foreground">Nenhuma estação cadastrada.</td></tr>}</tbody></table></div>
      </div>
      <div className="space-y-5">
        <div className="rounded-2xl border border-border/70 bg-background/60 p-5"><h3 className="mb-3 flex items-center gap-2 text-sm font-semibold text-foreground"><Gauge className="h-4 w-4 text-cyan-400" /> Envelope de Centro de Gravidade</h3>{envelope ? <ResponsiveContainer width="100%" height={280}><ComposedChart margin={{ top: 10, right: 16, bottom: 16, left: 0 }}><CartesianGrid stroke="hsl(var(--border))" strokeOpacity={0.25} /><XAxis type="number" dataKey="cg" domain={[envelope.cgFwd - 0.15, envelope.cgAft + 0.15]} tickFormatter={(value) => Number(value).toFixed(2)} stroke="hsl(var(--muted-foreground))" fontSize={11} /><YAxis type="number" dataKey="peso" domain={[Math.round(envelope.pesoMin * 0.9), Math.round(envelope.mtow * 1.05)]} stroke="hsl(var(--muted-foreground))" fontSize={11} /><ReferenceArea x1={envelope.cgFwd} x2={envelope.cgAft} y1={envelope.pesoMin} y2={envelope.mtow} fill="#06b6d4" fillOpacity={0.12} stroke="#06b6d4" strokeOpacity={0.5} /><Tooltip contentStyle={{ background: 'hsl(var(--card))', border: '1px solid hsl(var(--border))', borderRadius: 10, fontSize: 12 }} /><Scatter name="Referência" data={[{ cg: calc.cg, peso: calc.totalWeight }]} fill="#10b981" shape="circle" /></ComposedChart></ResponsiveContainer> : <p className="py-12 text-center text-xs text-muted-foreground">Informe MTOW e os limites de CG para desenhar o envelope.</p>}<p className="mt-2 text-center text-[11px] text-muted-foreground">Ponto de referência: CG {fmt(calc.cg, 3)} m · {fmt(calc.totalWeight, 1)} kg</p></div>

        <AircraftCGIndicator calc={calc} />

        <div className="rounded-2xl border border-border/70 bg-background/60 p-5"><div className="mb-3 flex items-center justify-between"><div><h3 className="text-sm font-semibold text-foreground">Limites certificados</h3><p className="text-xs text-muted-foreground">Usados como base dos carregamentos dos pilotos.</p></div><button onClick={saveLimits} disabled={savingLimits} className="inline-flex items-center gap-1.5 rounded-lg bg-ctm-teal px-3 py-1.5 text-xs font-semibold text-[hsl(var(--ctm-navy))] disabled:opacity-50 print:hidden">{savingLimits ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Save className="h-3.5 w-3.5" />} Salvar</button></div><div className="grid grid-cols-2 gap-3">{LIMITES.map((limit) => <label key={limit.key} className="block"><span className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-muted-foreground">{limit.label}</span><input className="ctm-input w-full" inputMode="decimal" value={limits[limit.key] ?? ''} onChange={(e) => setLimits((current) => ({ ...current, [limit.key]: e.target.value }))} /></label>)}</div></div>
      </div>
    </div>
  </>;
}
...
function LoadingsView({ loadings, showNewLoading, openNewLoading, setShowNewLoading, loadingForm, setLoadingForm, loadingItems, updateLoadingItem, loadingCalc, justification, setJustification, finalizeLoading, savingLoading }: any) {
  return <div className="space-y-5"><div className="flex flex-col gap-3 rounded-2xl border border-border/70 bg-background/60 p-5 sm:flex-row sm:items-center sm:justify-between"><div><h3 className="text-lg font-bold text-foreground">Carregamentos dos voos</h3><p className="text-xs text-muted-foreground">A ficha técnica é copiada como snapshot. O piloto informa somente os pesos do voo.</p></div><button onClick={openNewLoading} className="inline-flex items-center gap-1.5 whitespace-nowrap rounded-full border border-cyan-400/35 bg-transparent px-4 py-1.5 text-sm font-medium text-cyan-300 transition-colors hover:bg-cyan-400/10"><Plus className="h-3.5 w-3.5" /> Novo Carregamento</button></div>
    ...
    <div className="overflow-x-auto rounded-2xl border border-border/70 bg-background/60"><table className="w-full text-sm"><thead><tr className="border-b border-border text-left text-[11px] uppercase tracking-wide text-muted-foreground"><th className="px-4 py-3">Data</th><th className="px-4 py-3">Voo</th><th className="px-4 py-3">Responsável</th><th className="px-4 py-3 text-right">Peso</th><th className="px-4 py-3 text-right">CG</th><th className="px-4 py-3 text-right">Situação</th></tr></thead><tbody>{loadings.map((loading: any) => <tr key={loading.id} className="border-b border-slate-900/70"><td className="px-4 py-3 text-muted-foreground">{dateBR(loading.data_voo)}</td><td className="px-4 py-3 text-foreground">{loading.numero_voo || 'Sem número'}</td><td className="px-4 py-3 text-muted-foreground">{loading.piloto_responsavel}</td><td className="px-4 py-3 text-right text-muted-foreground">{fmt(n(loading.peso_total_kg), 1)} kg</td><td className="px-4 py-3 text-right text-muted-foreground">{fmt(n(loading.cg_calculado), 3)} m</td><td className="px-4 py-3 text-right"><span className={cn('inline-flex items-center gap-1 rounded-full border px-2.5 py-1 text-xs', loading.dentro_dos_limites ? 'border-emerald-500/30 bg-emerald-500/10 text-emerald-400' : 'border-amber-500/30 bg-amber-500/10 text-amber-400')}>{loading.dentro_dos_limites ? <CheckCircle2 className="h-3.5 w-3.5" /> : <AlertTriangle className="h-3.5 w-3.5" />} {loading.dentro_dos_limites ? 'Dentro do envelope' : 'Com justificativa'}</span></td></tr>)}{loadings.length === 0 && <tr><td colSpan={6} className="py-12 text-center text-sm text-muted-foreground">Nenhum carregamento finalizado.</td></tr>}</tbody></table></div>
  </div>;
}
...
function LoadingForm({ setShowNewLoading, loadingForm, setLoadingForm, loadingItems, updateLoadingItem, loadingCalc, justification, setJustification, finalizeLoading, savingLoading }: any) {
  return <div className="rounded-2xl border border-cyan-500/20 bg-background/80 p-5"><div className="mb-4 flex items-center justify-between"><div><h3 className="text-base font-bold text-foreground">Novo carregamento</h3><p className="text-xs text-muted-foreground">Braços e limites vieram da ficha técnica e não podem ser alterados aqui.</p></div><button onClick={() => setShowNewLoading(false)}><X className="h-5 w-5 text-muted-foreground" /></button></div><div className="grid grid-cols-1 gap-3 sm:grid-cols-3"><label><span className="mb-1 block text-[10px] uppercase tracking-wide text-muted-foreground">Data do voo</span><input type="date" className="ctm-input w-full" value={loadingForm.data_voo} onChange={(e) => setLoadingForm((form: any) => ({ ...form, data_voo: e.target.value }))} /></label><label><span className="mb-1 block text-[10px] uppercase tracking-wide text-muted-foreground">Número do voo</span><input className="ctm-input w-full" placeholder="Ex.: 1234" value={loadingForm.numero_voo} onChange={(e) => setLoadingForm((form: any) => ({ ...form, numero_voo: e.target.value }))} /></label><label><span className="mb-1 block text-[10px] uppercase tracking-wide text-muted-foreground">Combustível (litros)</span><input type="number" min="0" className="ctm-input w-full" placeholder="0" value={loadingForm.fuel_litros} onChange={(e) => setLoadingForm((form: any) => ({ ...form, fuel_litros: e.target.value }))} /></label></div><div className="mt-5 overflow-x-auto"><table className="w-full text-xs"><thead><tr className="border-b border-border text-[11px] uppercase tracking-wide text-muted-foreground"><th className="px-2 py-2 text-left">Estação</th><th className="px-2 py-2 text-right">Peso informado (kg)</th><th className="px-2 py-2 text-right">Braço travado (m)</th><th className="px-2 py-2 text-right">Momento</th></tr></thead><tbody>{loadingItems.map((item: any) => <tr key={item.estacao_id} className="border-b border-slate-900/70"><td className="px-2 py-2 text-foreground">{item.descricao}<span className="ml-2 text-[10px] uppercase text-muted-foreground">{item.categoria}</span></td><td className="px-2 py-1.5 text-right"><input type="number" min="0" className="w-28 rounded border border-border bg-card px-2 py-1 text-right text-foreground outline-none focus:border-cyan-500" placeholder="0" value={item.peso_kg} onChange={(e) => updateLoadingItem(item.estacao_id, e.target.value)} /></td><td className="px-2 py-2 text-right text-muted-foreground">{fmt(item.braco_posicao, 3)}</td><td className="px-2 py-2 text-right text-muted-foreground">{fmt(n(item.peso_kg) * n(item.braco_posicao), 1)}</td></tr>)}</tbody></table></div><div className="mt-5 grid grid-cols-2 gap-3 lg:grid-cols-5"><Kpi label="Peso total" value={`${fmt(loadingCalc.totalWeight, 1)} kg`} /><Kpi label="ZFW" value={`${fmt(loadingCalc.zfw, 1)} kg`} /><Kpi label="CG" value={`${fmt(loadingCalc.cg, 3)} m`} /><Kpi label="MTOW" value={`${fmt(loadingCalc.mtow, 1)} kg`} /><Kpi label="Resultado" value={loadingCalc.dentroLimites ? 'Aprovado' : 'Verificar'} /></div>

    <div className="mt-5">
      <AircraftCGIndicator calc={loadingCalc} />
    </div>

    {loadingCalc.alerts.length > 0 && <div className="mt-4 space-y-2">{loadingCalc.alerts.map((alert: any, index: number) => <div key={index} className={cn('flex items-center gap-2 rounded-xl border px-3 py-2 text-xs', alert.level === 'error' ? 'border-red-500/25 bg-red-500/10 text-red-400' : 'border-amber-500/25 bg-amber-500/10 text-amber-400')}><AlertTriangle className="h-4 w-4 shrink-0" /> {alert.msg}</div>)}</div>}{loadingCalc.alerts.length > 0 && <label className="mt-4 block"><span className="mb-1 block text-[10px] font-semibold uppercase tracking-wide text-amber-400">Justificativa operacional obrigatória</span><textarea className="ctm-input min-h-20 w-full" value={justification} onChange={(e) => setJustification(e.target.value)} placeholder="Descreva a autorização e a ação operacional prevista." /></label>}<div className="mt-5 flex justify-end gap-2"><button onClick={() => setShowNewLoading(false)} className="rounded-lg px-3 py-2 text-xs text-muted-foreground hover:text-foreground">Cancelar</button><button onClick={finalizeLoading} disabled={savingLoading} className="inline-flex items-center gap-1.5 rounded-lg bg-ctm-teal px-4 py-2 text-xs font-semibold text-[hsl(var(--ctm-navy))] disabled:opacity-50">{savingLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <CheckCircle2 className="h-3.5 w-3.5" />} Finalizar carregamento</button></div></div>;
}

function Kpi({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return <div className="rounded-xl border border-border/60 bg-card/40 px-4 py-3"><p className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">{label}</p><p className="mt-1 text-lg font-bold tracking-tight text-foreground">{value}</p>{sub && <p className="text-[10px] text-muted-foreground">{sub}</p>}</div>;
}