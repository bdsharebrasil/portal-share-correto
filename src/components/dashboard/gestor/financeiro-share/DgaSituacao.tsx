...
export default function DgaSituacao({ movimentacoes, socios, onChanged }: { movimentacoes:any[]; socios:any[]; onChanged?: () => void }) {
  ...
  <div className="rounded-2xl border border-border bg-card/60 overflow-hidden"><div className="p-4 border-b border-border"><div className="font-bold">Extrato do cliente para DGA</div><div className="text-xs text-muted-foreground">Movimentações vinculadas ao caixa DGA.</div></div><div className="overflow-x-auto"><table className="w-full text-xs"><thead><tr className="bg-muted/30 text-[10px] uppercase tracking-wider text-muted-foreground"><th className="text-left px-4 py-3">Data</th><th className="text-left px-4 py-3">Descrição</th><th className="text-left px-4 py-3">Categoria</th><th className="text-left px-4 py-3">Conta</th><th className="text-left px-4 py-3">Pago por</th><th className="text-right px-4 py-3">Valor</th><th className="text-left px-4 py-3">Status</th></tr></thead><tbody>{dga.slice().sort((a,b)=>String(dateOf(b)||"").localeCompare(String(dateOf(a)||""))).map(m=><tr key={m.id} className="border-t border-border/60 hover:bg-muted/20"><td className="px-4 py-3 whitespace-nowrap">{dateOf(m)?new Date(String(dateOf(m)).slice(0,10)+"T00:00:00").toLocaleDateString("pt-BR"):"—"}</td><td className="px-4 py-3 font-semibold">{m.descricao||"—"}</td><td className="px-4 py-3 text-muted-foreground">{m.categoria_nome||"—"}</td><td className="px-4 py-3 text-muted-foreground">{m.conta_bancaria||"—"}</td><td className="px-4 py-3 text-muted-foreground">{m.socios_nome||m.pago_por||"—"}</td><td className="px-4 py-3 text-right font-bold">{formatBRL(valueOf(m))}</td><td className="px-4 py-3 text-muted-foreground">{m.status||"—"}</td></tr>)}</tbody></table>{dga.length===0&&<div className="p-10 text-center text-sm text-muted-foreground">Nenhum lançamento DGA.</div>}</div></div>
