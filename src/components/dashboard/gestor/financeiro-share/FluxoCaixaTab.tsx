...
export default function FluxoCaixaTab(){
  ...
  const [busca,setBusca]=useState(""); const [mes,setMes]=useState(""); const [caixa,setCaixa]=useState<"todos"|"share"|"cliente"|"dga">("share");
  ...
  const catMap=useMemo(()=>{const m=new Map<string,any>();(data.categorias||[]).forEach((c:any)=>{m.set(c.id,c);m.set(String(c.nome||"").toLowerCase(),c)});return m},[data.categorias]);
  const grupoDe=useCallback((m:any)=>{const c=catMap.get(m?.categoria_id)||catMap.get(String(m?.categoria_nome||"").toLowerCase());return c?.grupo_categoria||c?.nome||m?.categoria_nome||"Sem categoria"},[catMap]);

  const movs=useMemo(()=>data.movimentacoes.filter((m:any)=>{if(String(m.status||'').toLowerCase()==='cancelado')return false;if(caixa==='dga'&&!isDga(m))return false;if(caixa==='share'&&(!isShare(m)||isEntrada(m)))return false;if(caixa==='cliente'&&(isDga(m)||isShare(m)))return false;if(mes&&String(dateOf(m)||'').slice(0,7)!==mes)return false;if(busca){const q=busca.toLowerCase();const text=`${m.descricao||''} ${m.fornecedor_nome||''} ${m.numero_doc||''} ${m.pago_por||''}`.toLowerCase();if(!text.includes(q))return false}return true}),[data.movimentacoes,caixa,mes,busca]);
  ...
  const evolucaoMensal=useMemo(()=>Array.from({length:6},(_,index)=>{const date=new Date(new Date().getFullYear(),new Date().getMonth()-5+index,1);const periodo=format(date,"yyyy-MM");const transacoes=data.movimentacoes.filter((m:any)=>String(m.status||"").toLowerCase()!=="cancelado"&&String(dateOf(m)||"").slice(0,7)===periodo);const entradas=transacoes.filter((m:any)=>!isDga(m)&&isEntrada(m)).reduce((s,m)=>s+valueOf(m),0);const saidas=transacoes.filter((m:any)=>!isDga(m)&&!isEntrada(m)).reduce((s,m)=>s+valueOf(m),0);return{periodo,label:format(date,"MMM",{locale:ptBR}),entradas,saidas,saldo:entradas-saidas}}),[data.movimentacoes]);
  ...
  return <div className="space-y-5">
    ...
    <div className="flex flex-col xl:flex-row xl:items-center gap-3 justify-between"><div><div className="text-xs font-bold uppercase tracking-wider text-muted-foreground">Financeiro</div><h2 className="text-2xl font-bold">Fluxo de Caixa</h2><p className="text-sm text-muted-foreground">Share, clientes e DGA com regras financeiras separadas.</p></div><div className="flex flex-wrap gap-2"><button onClick={()=>load()} className="rounded-lg border border-border px-3 py-2 text-xs font-semibold"><RefreshCw className="inline h-3.5 w-3.5 mr-1"/>Atualizar</button>{(aba==="caixa"||aba==="clientes")&&<button onClick={()=>{setNewCaixa(aba==="clientes"?"cliente":"share");setShowNew(true)}} className="rounded-lg bg-primary px-3 py-2 text-xs font-semibold text-primary-foreground"><Plus className="inline h-3.5 w-3.5 mr-1"/>Nova movimentação</button>}</div></div>
    <FinanceiroResumo resumo={resumo}/>
    <div className="flex flex-wrap gap-2 border-b border-border pb-2">{([["visao","Visão Geral"],["caixa","Caixa"],["clientes","Caixa Clientes"],["dga","DGA "]] as const).map(([k,l])=><button key={k} onClick={()=>setAba(k)} className={`rounded-lg border px-4 py-2 text-sm font-semibold ${aba===k?"border-primary bg-primary/10 text-primary":"border-border text-muted-foreground"}`}>{l}</button>)}</div>
    {showNew&&((aba==="caixa"&&newCaixa==="share")||(aba==="clientes"&&newCaixa==="cliente"))&&<div className="rounded-2xl border border-border bg-card/60 p-4">{newCaixa==="cliente"?<NovaDespesaClienteForm onCancel={()=>setShowNew(false)} onSaved={async()=>{setShowNew(false);await load()}}/>:<NovaDespesaShareForm onCancel={()=>setShowNew(false)} onSaved={async()=>{setShowNew(false);await load()}}/>}</div>}
    {aba==="visao"&&<>
      ...
      <div className="rounded-2xl border border-border bg-card/60 p-5"><div className="flex items-center gap-2"><BarChart3 className="h-4 w-4 text-primary"/><div className="font-bold">Evolução mensal</div></div><div className="mt-5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">{evolucaoMensal.map((item)=><div key={item.periodo} className="rounded-xl border border-border/70 bg-background/30 p-3"><div className="text-center text-xs font-semibold capitalize text-muted-foreground">{item.label}</div><div className="mt-3 flex h-32 items-end justify-center gap-2"><div title={`Entradas: ${formatBRL(item.entradas)}`} className="w-5 rounded-t bg-emerald-400/80" style={{height:`${item.entradas?Math.max(8,item.entradas/maiorMovimento*100):2}%`}}/><div title={`Saídas: ${formatBRL(item.saidas)}`} className="w-5 rounded-t bg-red-400/80" style={{height:`${item.saidas?Math.max(8,item.saidas/maiorMovimento*100):2}%`}}/></div><div className={`mt-2 text-center text-[11px] font-bold ${item.saldo>=0?"text-emerald-400":"text-red-400"}`}>{formatBRL(item.saldo)}</div><div className="text-center text-[10px] text-muted-foreground">{formatMesAno(item.periodo)}</div></div>)}</div><div className="mt-4 flex justify-center gap-4 text-[11px] text-muted-foreground"><span><i className="mr-1 inline-block h-2 w-2 rounded-full bg-emerald-400/80"/>Entradas</span><span><i className="mr-1 inline-block h-2 w-2 rounded-full bg-red-400/80"/>Saídas</span></div></div>
