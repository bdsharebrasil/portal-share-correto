...
export default function FluxoCaixaTab(){
  ...
  return <div className="space-y-5">
    ...
    <div className="flex flex-wrap gap-2 border-b border-border pb-2">{([["visao","Visão Geral"],["caixa","Caixa"],["clientes","Situação dos Clientes"],["dga","DGA "]] as const).map(([k,l])=><button key={k} onClick={()=>setAba(k)} className={`rounded-lg border px-4 py-2 text-sm font-semibold ${aba===k?"border-primary bg-primary/10 text-primary":"border-border text-muted-foreground"}`}>{l}</button>)}</div>
    {aba==="visao"&&<>
