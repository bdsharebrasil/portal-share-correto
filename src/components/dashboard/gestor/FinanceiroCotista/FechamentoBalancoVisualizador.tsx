...
export function FechamentoBalancoVisualizador({ aeronaveId, ano: anoProp, meses, onClose }: Props) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 antialiased print:bg-white print:text-black">
      ...
      {/* HEADER FIXO */}
      <div className="sticky top-0 z-30 border-b border-slate-200 bg-white/90 backdrop-blur-xl shadow-sm print:static print:border-none print:shadow-none print:bg-white">
        <div className="mx-auto flex max-w-[1400px] flex-col gap-3 px-4 py-3 sm:px-6 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="mb-0.5 text-[10px] uppercase font-bold tracking-widest text-slate-400">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <div className="flex items-center gap-2 rounded-lg border border-slate-200 bg-slate-50 px-3.5 py-1.5">
              ...
              <span className="text-sm font-semibold text-blue-700">{mesLabel}</span>
            </div>
            ...
            {onClose && (
              <button
                onClick={onClose}
                className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-600 transition-all hover:bg-slate-50 active:scale-[0.97] print:hidden"
              >
                Fechar
              </button>
            )}
      ...
      {!isLoading && (
        <div className="mx-auto max-w-[1400px] p-4 sm:p-6 print:p-0">
              ...
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <div className="overflow-x-auto rounded-xl border border-slate-200 bg-white shadow-sm">
                  <table className="w-full border-collapse text-sm">
                    <thead>
                      <tr>
                        <th className="border-b border-r border-slate-200 bg-slate-50 p-2" />
                        <th colSpan={cotistas.length} className="border-b border-slate-200 bg-amber-50 py-2 text-center text-xs font-bold uppercase tracking-widest text-amber-700">
                          Devem pagar para (A Receber De)
                        </th>
                      </tr>
                      <tr className="bg-slate-50 text-xs font-semibold uppercase text-slate-500">
                        <th className="border-b border-r border-slate-200 py-3 px-4 text-left">
                          Quem Pagou ↓
                        </th>
                        {cotistas.map((c) => (
                          <th key={c.id} className="border-b border-slate-200 py-3 px-4 text-center">
                            {c.nome}
                          </th>
                        ))}
                      </tr>
                      ...
                      {saldos.map((row) => {
                        ...
                        return (
                          <tr key={row.id} className="hover:bg-slate-50/50">
                            <td className="border-b border-r border-slate-200 bg-slate-50/50 py-3 px-4 font-semibold text-slate-700">
                              {row.nome}
                            </td>
                            {saldos.map((col) => {
                              if (row.id === col.id) {
                                return <td key={col.id} className="border-b border-slate-200 py-3 px-4 text-center text-slate-400">—</td>;
                              }
                              let val = 0;
                              ...
                              return (
                                <td key={col.id} className={`border-b border-slate-200 py-3 px-4 text-right font-mono text-sm ${val > 0.01 ? "text-emerald-700 font-bold bg-emerald-50/30" : "text-slate-400"}`}>
                                  {val > 0.01 ? BRL(val) : "R$ —"}
                                </td>
                              );
                            })}
                ...
                <div className="grid grid-cols-2 gap-4">
                  {saldos.map((s) => (
                    <div key={s.id} className={`rounded-xl border p-5 shadow-sm ${s.saldo >= 0 ? "border-emerald-200 bg-emerald-50/50" : "border-red-200 bg-red-50/50"}`}>
                      <p className="mb-3 text-xs font-bold uppercase tracking-widest text-slate-500">
                        {s.nome}
                      </p>
                      <div className="flex items-center gap-2 mb-2">
                        {s.saldo >= 0 ? <TrendingUp className="h-5 w-5 text-emerald-600"/> : <TrendingDown className="h-5 w-5 text-red-600"/>}
                        <p className={`text-2xl font-bold ${s.saldo >= 0 ? "text-emerald-700" : "text-red-700"}`}>
                      ...
                      <div className="space-y-1.5 border-t border-slate-200/60 pt-3 text-xs">
                        <div className="flex justify-between text-slate-600"><span>Pagou:</span><span className="font-mono font-medium">{BRL(s.pagou)}</span></div>
                        <div className="flex justify-between text-slate-600"><span>Deve:</span><span className="font-mono font-medium">{BRL(s.deve)}</span></div>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  <table className="w-full border-collapse text-xs">
                    <thead>
                      <tr className="bg-slate-50 text-[10px] uppercase tracking-widest text-slate-500">
                        <th colSpan={9} className="border-b border-r border-slate-200 py-3 px-4 text-left font-bold">
                          Qualificação da Despesa
                        </th>
                        {cotistas.map((c) => (
                          <th key={`g-pct-${c.id}`} className="border-b border-slate-200 py-3 px-2 text-center font-bold text-slate-400">%</th>
                        ))}
                        {cotistas.map((c) => (
                          <th key={`g-rat-${c.id}`} className="border-b border-l border-slate-200 py-3 px-2 text-center font-bold text-blue-700">
                            {c.nome.split(" ")[0]}
                          </th>
                        ))}
                      </tr>
                      <tr className="bg-slate-100 text-[10px] font-bold uppercase text-slate-600">
                        ...
                        <Th>Prazo</Th>
                        <Th>Pago Por</Th>
                        <Th className="border-r border-slate-200 text-slate-800" right>Total</Th>
                        {cotistas.map((c) => <Th key={`h-pct-${c.id}`} right>{abrev(c.nome)} %</Th>)}
                        {cotistas.map((c) => <Th key={`h-rat-${c.id}`} className="border-l border-slate-200 text-blue-800" right>R$</Th>)}
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {despesasAgrupadas.map(({ ref, rateios }, i) => {
                        ...
                        return (
                          <tr key={ref.despesa_id || ref.id} className="hover:bg-slate-50 transition-colors">
                            ...
                            <Td className="border-r border-slate-200 font-bold text-slate-800" mono right>
                            ...
                            {cotistas.map((c) => {
                              ...
                              const val = Number(r?.valor_rateado ?? 0);
                              return (
                                <Td key={`rat-${c.id}`} className="border-l border-slate-200" dim={val === 0} fontSemibold={val > 0} mono right>
                                  {val > 0 ? BRL(val) : "—"}
                                </Td>
                              );
                            })}
                    ...
                    <tfoot>
                      <tr className="bg-slate-800 text-xs font-bold text-white">
                        ...
                        {cotistas.map((c) => <td key={`ft-pct-${c.id}`} className="py-3 px-4 text-right text-slate-400">—</td>)}
                        {cotistas.map((c) => (
              ...
              <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-4">
                {resumoCotistas.map((c) => (
                  <div key={c.id} className="rounded-xl border border-slate-200 bg-white shadow-sm overflow-hidden">
                    <div className="bg-blue-50 px-5 py-4 border-b border-slate-200">
                      <p className="text-[10px] font-bold uppercase tracking-widest text-blue-600 mb-1">Cotista</p>
                      <p className="text-lg font-bold text-slate-800">{c.nome}</p>
                    </div>
                    <div className="p-5 space-y-4 text-sm">
                      ...
                      <div className="border-t border-slate-100 pt-3 flex justify-between items-center bg-slate-50 p-2 rounded-lg">
                        <span className="text-slate-600 font-bold uppercase text-xs">Custo Total</span>
                        <span className="font-mono text-base font-bold text-blue-700">{BRL(c.total)}</span>
                      </div>
                      {(c.horas > 0 || c.litros > 0) && (
                        <div className="border-t border-slate-100 pt-4 space-y-2">
                          <p className="text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2">Indicadores Operacionais</p>
            ...
            <section>
              ...
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                  ...
                  <div className="flex h-6 overflow-hidden rounded-full bg-slate-100 mb-4">
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
                  ...
                  <div className="mt-5 rounded-lg bg-slate-50 p-4 border border-slate-100 text-xs text-slate-600 leading-relaxed">
          ...
          <div className={`space-y-8 animate-fade-in print:block print:space-y-8 ${activeTab === "graficos" ? "block" : "hidden"}`}>
              ...
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                  ...
                  <table className="w-full border-collapse text-sm">
                    <thead>
                      <tr className="bg-slate-50 text-xs font-bold uppercase text-slate-500">
                        ...
                        <Th className="text-slate-800" right>Total Geral</Th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-100">
                      {pivot.sortedCats.map((cat) => {
                        ...
                        return (
                          <tr key={cat} className="hover:bg-slate-50">
                            <Td className="font-bold text-slate-700 py-3 px-4" upper>{cat}</Td>
                            {cotistas.map((c) => (
                              <Td key={c.id} dim={!byC.get(c.id)} mono right>
                                {byC.get(c.id) ? BRL(byC.get(c.id)!) : "—"}
                              </Td>
                            ))}
                            <Td className="font-bold text-slate-800" mono right>{BRL(rowTotal)}</Td>
                          </tr>
                        );
                      })}
                    </tbody>
                    <tfoot>
                      <tr className="bg-blue-50 text-xs font-bold text-blue-900 border-t-2 border-blue-200">
                        <td className="py-3 px-4 uppercase">Total Geral</td>
                        {cotistas.map((c) => (
                          <td key={c.id} className="py-3 px-4 text-right font-mono text-blue-700">{BRL(pivot.cotTot.get(c.id) ?? 0)}</td>
                        ))}
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm flex flex-col gap-5">
                    ...
                    return (
                      <div key={cat} className="space-y-1.5">
                        <div className="flex justify-between text-xs">
                          ...
                          <span className="font-mono font-semibold text-slate-600">{BRL(rowTotal)}</span>
                        </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-slate-100 flex flex-wrap gap-3">
                    {cotistas.map((c, ci) => (
                      <div key={c.id} className="flex items-center gap-2 text-xs font-medium text-slate-600">
            ...
            <section>
              ...
              {ranking.length === 0 ? (
                ...
                <div className="rounded-xl border border-slate-200 bg-white shadow-sm divide-y divide-slate-100">
                  {ranking.map((r, i) => (
                    <div key={r.nome} className="flex items-center gap-4 px-5 py-4 hover:bg-slate-50">
                      <span className="w-8 text-center text-sm font-bold text-slate-400">{i + 1}º</span>
                      <span className="flex-1 truncate text-sm font-bold uppercase text-slate-700">{r.nome}</span>
                      <span className="w-32 text-right font-mono text-sm font-semibold text-slate-800">{BRL(r.valor)}</span>
                      <span className="w-16 shrink-0 rounded-lg bg-blue-50 px-2 py-1 text-center text-xs font-bold text-blue-700">
                        {NUM(r.pct, 0)}%
                      </span>
                      <div className="hidden h-2 w-48 shrink-0 overflow-hidden rounded-full bg-slate-100 sm:block">
            ...
            <section>
              ...
              <div className="grid grid-cols-1 gap-6 lg:grid-cols-5">
                <div className="lg:col-span-2 space-y-5">
                  <div className="rounded-xl bg-blue-600 p-6 text-white shadow-md">
                    ...
                    <p className="font-mono text-3xl font-bold">
                      {BRL(projecao.taxaProjecao)} <span className="text-lg font-normal text-blue-200">/ hora</span>
                    </p>
                  ...
                  <div className="rounded-xl border border-slate-200 bg-white p-5 text-sm text-slate-600 space-y-3 shadow-sm">
                    <p className="text-xs font-bold uppercase tracking-widest text-slate-800">Como analisar:</p>
                    ...
                    {projecao.pontoOtimo && (
                      <div className="mt-3 rounded-lg border border-emerald-200 bg-emerald-50 p-3 text-emerald-800 font-medium">
                ...
                <div className="lg:col-span-3 max-h-[500px] overflow-auto rounded-xl border border-slate-200 shadow-sm bg-white">
                  <table className="w-full border-collapse text-xs">
                    <thead className="sticky top-0 z-10">
                      <tr className="bg-slate-100 text-[10px] font-bold uppercase text-slate-600 shadow-sm">
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {projecao.linhas.map((l) => {
                        ...
                        return (
                          <tr key={l.horas} className={destaque ? "bg-emerald-50" : "hover:bg-slate-50"}>
                            <Td className={`font-bold py-2 ${destaque ? 'text-emerald-700' : 'text-slate-800'}`} mono right>{l.horas}h</Td>
                            <Td dim mono right>{BRL(l.custoVariavel)}</Td>
                            <Td dim mono right>{BRL(l.custoFixoHora)}</Td>
                            <Td className={`font-bold ${destaque ? 'text-emerald-700' : 'text-blue-700'}`} mono right>{BRL(l.custoHora)}</Td>
                            <Td className="text-slate-400 font-medium" mono right>{NUM(l.indice, 2)}x</Td>
                          </tr>
                        );
                      })}
                    </tbody>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {data.voos.map((v) => (
                        <tr key={v.id} className="hover:bg-slate-50">
                          ...
                          <Td className="font-bold text-slate-700" mono right>{v.tempo_voo ? hhMM(v.tempo_voo) : v.tempo_total ? hhMM(v.tempo_total) : "—"}</Td>
                          <Td mono right dim>{v.pousos_total ?? "—"}</Td>
                          <Td mono right className="text-amber-600 font-medium">{v.combustivel_adicionado ? `${NUM(v.combustivel_adicionado, 0)} L` : "—"}</Td>
          ...
          <footer className="mt-12 border-t border-slate-200 pt-6 text-center text-xs text-slate-500 font-medium">
...
function Vazio({ texto }: { texto: string }) {
  return (
    <div className="rounded-xl border border-dashed border-slate-300 bg-slate-50 py-16 text-center text-sm font-medium text-slate-500">
      {texto}
    </div>
  );
}
...
function KpiCard({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm transition-all hover:shadow-md">
      ...
      <p className="font-mono text-2xl font-bold tracking-tight text-blue-700">
        {value}
      </p>
    </div>
  );
...
function LinhaResumo({ label, value, isText, bold }: { label: string; value: number | string; isText?: boolean; bold?: boolean }) {
  return (
    <div className="flex justify-between items-center gap-3 text-sm">
      ...
      <span className={`font-mono ${bold ? "font-bold text-slate-800" : "text-slate-600"}`}>
...
function Th({ children, right, className = "" }: { children?: React.ReactNode; right?: boolean; className?: string }) {
  return (
    <th className={["py-3 px-3 border-b border-slate-200 whitespace-nowrap align-middle", right ? "text-right" : "text-left", className].join(" ")}>
      {children}
    </th>
  );
}
  ...
  return (
    <td
      ...
      className={[
        ...
        mono ? "font-mono" : "",
        upper ? "uppercase" : "",
        dim ? "text-slate-500" : "text-slate-700",
