...
function BalancoPage() {
  ...
  const previsao = useMemo(() => {
    ...
    const diaDoAno = anoCorrente

      ? Math.floor((hoje.getTime() - new Date(ano, 0, 1).getTime()) / 86_400_000) + 1

      : 365;

    const diasDecorridos = Math.max(1, Math.min(365, diaDoAno));

    const totalDias = 365;



    const custoYtd = serieMensal.reduce((s, p) => s + p.custo, 0);

    const horasYtd = serieMensal.reduce((s, p) => s + p.horas, 0);

    const voosYtd = serieMensal.reduce((s, p) => s + p.voos, 0);

    const pousosYtd = voos.reduce((s, v) => s + (Number(v.pousos_total) || 0), 0);



    const fator = anoCorrente ? totalDias / diasDecorridos : 1;
    ...
    return {
      ...
      custoProj: custoYtd * fator,

      horasProj: horasYtd * fator,

      voosProj: voosYtd * fator,

      pousosProj: pousosYtd * fator,

      diasDecorridos,

    };

  }, [serieMensal, voos, ano]);



  /* ── year in review (todo o ano selecionado) ── */

  const yearReview = useMemo(() => {

    const totalHoras = voos.reduce((s, v) => s + (Number(v.tempo_total) || Number(v.tempo_voo) || 0), 0);

    const totalVoos = voos.length;

    const totalPousos = voos.reduce((s, v) => s + (Number(v.pousos_total) || 0), 0);
    ...
    voos.forEach((v) => {
      ...
      const d = (v.aerodromo_chegada || "").toUpperCase().trim();
    ...
    const destinoFav = Array.from(destinos.entries()).sort((a, b) => b[1] - a[1])[0];



    // ranking cotistas por horas

    const rankHoras = new Map<string, number>();

    cotistas.forEach((c) => rankHoras.set(c.id, 0));
    ...
    voos.forEach((v) => {
      ...
      rankHoras.set(k, (rankHoras.get(k) || 0) + (Number(v.tempo_total) || Number(v.tempo_voo) || 0));

    });

    const ranking = Array.from(rankHoras.entries())

      .map(([id, h]) => ({ cotista: cotistas.find((c) => c.id === id)!, horas: h }))
    ...
    return { totalHoras, totalVoos, totalPousos, destinoFav, ranking };
    ...
    <div className="min-h-screen">
      ...
      <div className="sticky top-0 z-40 border-b border-border/60 bg-background/80 backdrop-blur-xl">
        ...
        <div className="mx-auto flex max-w-[1400px] flex-wrap items-center gap-3 px-4 py-3 sm:px-6 lg:px-8">
          ...
          <Link to="/dashboard" className="rounded-md px-2.5 py-1 text-xs text-muted-foreground transition-colors hover:text-foreground hover:bg-muted/50">
          ...
          <div className="ml-auto flex flex-wrap items-center gap-2">
            ...
            <div className="flex items-center gap-1 rounded-md border border-input bg-background px-2 py-1 text-[11px] text-muted-foreground">
              ...
              {(dataDe || dataAte) && (
                ...
                <button onClick={() => { setDataDe(""); setDataAte(""); }} className="ml-1 rounded px-1 text-[10px] hover:bg-muted">limpar</button>
      ...
      <main className="mx-auto max-w-[1400px] space-y-10 px-4 pb-24 pt-8 sm:px-6 lg:px-8">

        {/* ── Hero ─────────────────────────────────────────── */}

        <section className="rounded-3xl border border-border/60 bg-gradient-to-br from-[oklch(0.22_0.05_255)] via-background to-background p-6 sm:p-10">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div>
              ...
              <p className="mt-2 max-w-xl text-sm text-muted-foreground">
                ...
                Espelho financeiro completo do mês: quanto a aeronave custou, quem ainda deve, quanto cada cotista gastou e se o caixa fechou positivo ou negativo.

              </p>

            </div>

            <div className="flex items-center gap-6 rounded-2xl border border-border/60 bg-card/50 px-5 py-4">
          ...
          <>
            ...
            <div>
              ...
              >
                ...
                <div className="overflow-hidden">
                  ...
                  <div className="mt-3 rounded-2xl border border-emerald-500/25 bg-emerald-500/[0.05] p-5">
                        ...
                        {entradasPorCotista.map((c) => {
                          ...
                          return (
                            ...
                            <div key={c.id} className="flex items-center gap-3">
                              ...
                              <div className="w-28 shrink-0 text-right text-sm font-semibold text-emerald-400">
            ...
            <div className={cn(
              ...
              {equilibrado
                ...
                : <Scale className="mt-0.5 h-6 w-6 text-amber-400" />}
            ...
            <ForecastCard previsao={previsao} ano={ano} matricula={activeAircraft?.matricula} />



            {/* ── Evolução mensal ─────────────────────────── */}

            <section className="motion-safe:animate-[fadeUp_0.6s_ease-out_both] rounded-2xl border border-border/60 bg-card/40 p-5">
              ...
              <div className="mb-4 flex items-center justify-between">
                ...
                <div>
                  ...
                  <h2 className="mt-0.5 text-lg font-semibold">Custo mensal — {ano}</h2>

                </div>

                <div className="text-xs text-muted-foreground">

                  Total acumulado: <span className="font-semibold text-foreground">{formatBRL(previsao.custoYtd)}</span>
            ...
            <section>
              ...
              <div className="mb-4">
                ...
                <h2 className="mt-0.5 text-lg font-semibold">Débito, crédito e saldo do mês</h2>
            ...
            <YearInReview ano={ano} review={yearReview} matricula={activeAircraft?.matricula} />
...
function SelectField({
  ...
  return (
    ...
    <select
      ...
      className="h-9 rounded-md border border-input bg-background px-3 text-xs transition-colors duration-150 hover:border-primary/50 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20 active:scale-[0.98]"
...
function StatMini({ label, value, tone }: { label: string; value: string; tone?: "primary" }) {
  ...
  return (
    ...
    <div>
      ...
      <div className={cn("mt-0.5 font-display text-lg", tone === "primary" && "text-primary")}>{value}</div>
  ...
  return (
    ...
    <div
      ...
      className={cn(
        ...
        "rounded-2xl border border-border/60 bg-card/50 p-5 transition-all duration-300 hover:-translate-y-0.5 hover:scale-[1.015] hover:border-primary/30 hover:shadow-[0_8px_30px_-12px_var(--primary)]",
    ...
    >
      ...
      <div className={cn("mt-3 font-display text-2xl", tone === "primary" && "text-primary")}>{value}</div>
  ...
  return (
    ...
    <div
      ...
      className={cn(
        ...
        "group cursor-pointer select-none rounded-2xl border border-border/60 bg-card/60 p-5 transition-all duration-300 hover:-translate-y-0.5 hover:scale-[1.015] hover:border-primary/30 hover:shadow-[0_8px_30px_-12px_var(--primary)]",
    ...
    >
      ...
      <div className="mt-4 grid grid-cols-2 gap-3">
        ...
        <div className="rounded-lg bg-muted/30 p-3">
          ...
          <div className="mt-0.5 font-semibold">{formatBRL(linha.debito)}</div>
        ...
        <div className="rounded-lg bg-muted/30 p-3">
          ...
          <div className="mt-0.5 font-semibold">{formatBRL(linha.credito)}</div>
      ...
      <div className="mt-3 flex items-center justify-end gap-1 text-[11px] font-medium text-muted-foreground/70 transition-colors duration-200 group-hover:text-primary">
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <input
            ...
            className="h-9 w-64 rounded-md border border-input bg-background px-3 text-xs transition-colors duration-150 hover:border-primary/50 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20"
            ...
            <table className="w-full text-sm">
              ...
              <thead className="bg-muted/40 text-[11px] uppercase tracking-wider text-muted-foreground">
                ...
                <tr>
                  ...
                  {showCotista && <th className="px-3 py-2 text-left">Cotista</th>}

                  {showCotista && <th className="px-3 py-2 text-right">% uso</th>}
              ...
              <tfoot className="bg-muted/30 text-xs">
                ...
                <tr className="border-t border-border/60">
                  ...
                  <td colSpan={showCotista ? 8 : 6} className="px-3 py-2 text-right uppercase tracking-widest text-muted-foreground">Total saídas</td>

                  <td className="px-3 py-2 text-right font-semibold">{formatBRL(totalSaida)}</td>
                ...
                {totalEntrada > 0 && (
                  ...
                  <tr>
                    ...
                    <td colSpan={showCotista ? 8 : 6} className="px-3 py-2 text-right uppercase tracking-widest text-muted-foreground">Total entradas</td>

                    <td className="px-3 py-2 text-right font-semibold text-emerald-400">{formatBRL(totalEntrada)}</td>
  ...
  return (
    ...
    <>
      ...
      >
        ...
        <td className="px-3 py-2 text-muted-foreground">{formatDate(linha.data)}</td>

        <td className="px-3 py-2">{linha.fornecedor || "—"}</td>

        <td className="px-3 py-2 text-muted-foreground">{linha.descricao || "—"}</td>

        <td className="px-3 py-2 text-muted-foreground">{linha.documento || "—"}</td>

        {showCotista && <td className="px-3 py-2 text-muted-foreground">{linha.cotistaNome || "—"}</td>}
        ...
        <td className={cn("px-3 py-2 text-right font-medium", linha.entrada ? "text-emerald-400" : "text-foreground")}>
  ...
  return (
    ...
    >
      ...
      <span className={cn(
        ...
        disponivel ? "bg-primary/15 text-primary" : "bg-muted/30 text-muted-foreground",
...
function DetalheLancamento({
  ...
  return (
    ...
    <div className="space-y-5 motion-safe:animate-[fadeUp_0.35s_ease-out_both]">
...
function YearInReview({
  ...
  review: {
    ...
    totalHoras: number; totalVoos: number; totalPousos: number;

    destinoFav?: [string, number];

    ranking: { cotista: Cotista; horas: number }[];
    ...
    <section className="rounded-3xl border border-border/60 bg-gradient-to-br from-[oklch(0.18_0.06_265)] via-[oklch(0.14_0.05_255)] to-background p-6 sm:p-10">
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        ...
        {review.ranking.length > 0 && (
          ...
          <SelectField value={String(cotistaIdx)} onChange={(v) => setCotistaIdx(Number(v))}>
...
function EstatisticasCotista({
  ...
  const adColor = stats.adimplencia >= 95 ? "#3fa998" : stats.adimplencia >= 60 ? "#e8b939" : "#e07a5f";
  ...
  return (
    <section className="space-y-4 rounded-2xl border border-border/60 bg-card/40 p-5">
      <header>
        ...
        <h2 className="mt-0.5 text-lg font-semibold">{cotista.nome}</h2>
      </header>

      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard icon={<Clock className="h-4 w-4" />} label="Horas utilizadas" value={formatHours(stats.horas)} />
        <StatCard icon={<CheckCircle2 className="h-4 w-4" />} label="Valor pago" value={formatBRL(stats.pago)} tone="success" />
        <button
          type="button"
          onClick={() => stats.pendenteItems.length && setShowPend((v) => !v)}
          disabled={!stats.pendenteItems.length}
          className={cn(
            "rounded-xl border border-border/50 bg-background/40 p-4 text-left transition-all",
        ...
        >
          <div className="flex items-center justify-between text-[11px] uppercase tracking-widest text-muted-foreground">
            <span className="flex items-center gap-2"><Wallet className="h-4 w-4" style={{ color: stats.pendente > 0 ? "#e8b939" : undefined }} /> Valor pendente</span>
            {stats.pendenteItems.length > 0 && (
              <ChevronDown className={cn("h-3.5 w-3.5 transition-transform", showPend && "rotate-180 text-amber-400")} />
            )}
          </div>
          <div className="mt-2 text-xl font-semibold tabular-nums" style={{ color: stats.pendente > 0 ? "#e8b939" : "var(--muted-foreground)" }}>
      ...
      {showPend && stats.pendenteItems.length > 0 && (
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/[0.05] p-4">
          <div className="mb-3 flex items-center gap-2 text-xs uppercase tracking-widest text-amber-400">
            ...
            <table className="w-full text-xs">
              <thead className="bg-muted/40 text-[10px] uppercase tracking-wider text-muted-foreground">
                <tr>
                  ...
                  <th className="px-3 py-2 text-right text-amber-400">Falta</th>
                </tr>
              </thead>
              <tbody>
                {stats.pendenteItems.map(({ r, base, pago: p, falta }) => (
                  <tr key={r.id} className="border-t border-border/30">
                    <td className="px-3 py-1.5">{formatDate(r.data_vencimento || r.data_pagamento)}</td>
                    <td className="px-3 py-1.5">{r.descricao_despesa || resolveCategoria(r.categoria_custo, categoriasMap)}</td>
                    <td className="px-3 py-1.5 text-muted-foreground">{r.fornecedor_nome || "—"}</td>
                    <td className="px-3 py-1.5 text-right tabular-nums">{formatBRL(base)}</td>
                    <td className="px-3 py-1.5 text-right tabular-nums text-muted-foreground">{formatBRL(p)}</td>
                    <td className="px-3 py-1.5 text-right tabular-nums font-semibold text-amber-400">{formatBRL(falta)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-border/50 p-4">
          <div className="mb-3 text-xs uppercase tracking-widest text-muted-foreground">Gastos por categoria</div>
...
function StatCard({
  ...
  const toneColor =
    accent ||
    (tone === "success" ? "var(--success)" : tone === "warning" ? "#e8b939" : "var(--primary)");
  return (
    <div className="rounded-xl border border-border/50 bg-background/40 p-4 transition-all hover:-translate-y-0.5 hover:shadow-lg">
      <div className="flex items-center gap-2 text-[11px] uppercase tracking-widest text-muted-foreground">
        <span style={{ color: toneColor }}>{icon}</span> {label}
      </div>
      <div className="mt-2 text-xl font-semibold tabular-nums" style={{ color: toneColor }}>
        {value}
      </div>
    </div>
  );
  ...
  return (
    <section className="rounded-2xl border border-border/60 bg-card/40 p-5">
      <header className="mb-4">
        ...
        <h2 className="mt-0.5 text-lg font-semibold">Custos fixos, variáveis, extras e projeção de custo/hora</h2>
      </header>
        ...
        <table className="w-full text-xs">
          <thead className="bg-muted/40 text-[10px] uppercase tracking-wider text-muted-foreground">
            <tr>
              ...
              <th className="px-3 py-2 text-right">Custos Fixos</th>
              <th className="px-3 py-2 text-right">Var. Hora</th>
              <th className="px-3 py-2 text-right">Var. Voo</th>
              <th className="px-3 py-2 text-right">Extras</th>
              <th className="px-3 py-2 text-right bg-primary/10 text-primary">Total</th>
              <th className="px-3 py-2 text-right">H Voadas</th>
              <th className="px-3 py-2 text-right">Pousos</th>
              <th className="px-3 py-2 text-right">Custo/H</th>
              <th className="px-3 py-2 text-right">Custo/Pouso</th>
            </tr>
          </thead>
          <tbody>
            {data.rows.map((r, i) => {
              ...
              return (
                <tr key={i} className={cn("border-t border-border/40", empty && "opacity-40")}>
                  <td className="px-3 py-1.5 font-medium">{MESES[i]}</td>
                  <td className="px-3 py-1.5 text-right tabular-nums">{formatBRL(r.fixo)}</td>
                  <td className="px-3 py-1.5 text-right tabular-nums">{formatBRL(r.varHora)}</td>
                  <td className="px-3 py-1.5 text-right tabular-nums">{formatBRL(r.varVoo)}</td>
                  <td className="px-3 py-1.5 text-right tabular-nums">{formatBRL(r.extra)}</td>
                  <td className="px-3 py-1.5 text-right tabular-nums font-semibold bg-primary/[0.06] text-primary">{formatBRL(tot)}</td>
                  <td className="px-3 py-1.5 text-right tabular-nums">{r.horas.toFixed(1)}</td>
                  <td className="px-3 py-1.5 text-right tabular-nums">{r.pousos}</td>
                  <td className="px-3 py-1.5 text-right tabular-nums">{cph ? formatBRL(cph) : "—"}</td>
                  <td className="px-3 py-1.5 text-right tabular-nums">{cpp ? formatBRL(cpp) : "—"}</td>
                </tr>
              );
            })}
          </tbody>
          <tfoot className="text-xs">
            <tr className="border-t border-border/60 bg-muted/30 font-semibold">
              ...
              <td className="px-3 py-2 text-right tabular-nums">{formatBRL(data.total.fixo)}</td>
              <td className="px-3 py-2 text-right tabular-nums">{formatBRL(data.total.varHora)}</td>
              <td className="px-3 py-2 text-right tabular-nums">{formatBRL(data.total.varVoo)}</td>
              <td className="px-3 py-2 text-right tabular-nums">{formatBRL(data.total.extra)}</td>
              <td className="px-3 py-2 text-right tabular-nums bg-primary/10 text-primary">{formatBRL(totalGeral)}</td>
              <td className="px-3 py-2 text-right tabular-nums">{data.total.horas.toFixed(1)}</td>
              <td className="px-3 py-2 text-right tabular-nums">{data.total.pousos}</td>
              <td className="px-3 py-2 text-right tabular-nums">{data.total.horas > 0 ? formatBRL(totalGeral / data.total.horas) : "—"}</td>
              <td className="px-3 py-2 text-right tabular-nums">{data.total.pousos > 0 ? formatBRL(totalGeral / data.total.pousos) : "—"}</td>
            </tr>
            <tr className="border-t border-border/60 text-muted-foreground">
              <td className="px-3 py-2">Média/mês ({data.mesesAtivos} {data.mesesAtivos > 1 ? "meses" : "mês"})</td>
              <td className="px-3 py-2 text-right tabular-nums">{formatBRL(data.media.fixo)}</td>
              <td className="px-3 py-2 text-right tabular-nums">{formatBRL(data.media.varHora)}</td>
              <td className="px-3 py-2 text-right tabular-nums">{formatBRL(data.media.varVoo)}</td>
              <td className="px-3 py-2 text-right tabular-nums">{formatBRL(data.media.extra)}</td>
              <td className="px-3 py-2 text-right tabular-nums text-primary">{formatBRL(mediaTotal)}</td>
              <td className="px-3 py-2 text-right tabular-nums">{data.media.horas.toFixed(1)}</td>
              <td className="px-3 py-2 text-right tabular-nums">{data.media.pousos.toFixed(1)}</td>
              <td className="px-3 py-2" />
              <td className="px-3 py-2" />
            </tr>
          </tfoot>
      ...
      <div className="mt-6 grid gap-4 lg:grid-cols-3">
        <div className="rounded-xl border border-primary/30 bg-primary/[0.05] p-4">
          ...
          <div className="mt-1 font-display text-2xl text-primary">{formatBRL(vMenor)}</div>
          <div className="text-[11px] text-muted-foreground">com {horasSim}h no período</div>
        </div>
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/[0.05] p-4">
          <div className="text-[10px] uppercase tracking-widest text-amber-400">V Maior</div>
          <div className="mt-1 font-display text-2xl text-amber-400">{formatBRL(vMaior)}</div>
          <div className="text-[11px] text-muted-foreground">com 1h no período</div>
        </div>
        <div className="rounded-xl border border-emerald-500/30 bg-emerald-500/[0.05] p-4">
          ...
          <div className="mt-1 font-display text-2xl text-emerald-400">{economia.toFixed(0)}%</div>
          <div className="text-[11px] text-muted-foreground">diluição do custo fixo</div>
        </div>
      </div>
      ...
      <div className="mt-4">
        ...
        <div className="mt-3 max-h-72 overflow-y-auto rounded-xl border border-border/50">
          <table className="w-full text-xs">
            <thead className="sticky top-0 bg-muted/40 text-[10px] uppercase tracking-wider text-muted-foreground">
              <tr>
                <th className="px-3 py-2 text-left">H Voadas</th>
                <th className="px-3 py-2 text-right">Custo Fixo/H</th>
                <th className="px-3 py-2 text-right">Custo Variável/H</th>
                <th className="px-3 py-2 text-right">Custo Extra/H</th>
                <th className="px-3 py-2 text-right bg-primary/10 text-primary">Custo Hora Total</th>
              </tr>
            </thead>
            <tbody>
              {projecao.map((p) => (
                <tr key={p.h} className="border-t border-border/40">
                  <td className="px-3 py-1.5 font-medium">{p.h}h</td>
                  <td className="px-3 py-1.5 text-right tabular-nums">{formatBRL(p.fixo)}</td>
                  <td className="px-3 py-1.5 text-right tabular-nums">{formatBRL(p.variavel)}</td>
                  <td className="px-3 py-1.5 text-right tabular-nums">{formatBRL(p.extra)}</td>
                  <td className="px-3 py-1.5 text-right tabular-nums font-semibold text-primary bg-primary/[0.06]">{formatBRL(p.total)}</td>
                </tr>
              ))}
            </tbody>
          </table>
