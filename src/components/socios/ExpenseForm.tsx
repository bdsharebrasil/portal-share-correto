...
export function ExpenseForm({ clienteId }: ExpenseFormProps) {
  ...
  const [existingReports, setExistingReports] = useState<Array<{id: string; report_number: string; status: string}>>([]);
  const [selectedReport, setSelectedReport] = useState<{id: string; report_number: string; status: string} | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // if linking to existing travel report make sure one is selected
    ...
    // Determine possible reference information for travel report linking
    ...
    if (form.category === 'viagem') {
      if (linkOption === 'existing' && selectedReport) {
        ...
        // create new travel report and insert first expense
        try {
          // fetch client name for number generation
  ...
  const assignedPartner = form.assignedPartnerCpf && form.assignedPartnerCpf !== 'none'
      ...
      <DialogContent
        ...
        <div className="px-8 py-6">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            {/* Travel report linking question */}
            {form.category === 'viagem' && (
              <FormSection label="Deseja vincular esta despesa a um relatório existente ou iniciar um novo relatório?">
                      ...
                      <SelectContent className="rounded-xl">
                        {existingReports.map((r) => (
                          <SelectItem key={r.id} value={r.id} className="py-3">
                            {r.report_number} {r.status && `(${r.status})`}
                          </SelectItem>
                        ))}
                      </SelectContent>
                    </Select>
              ...
              <FormSection label="Fornecedor">
                {isAbastecimento ? (
                  ...
                  <SearchableCombobox
                    ...
                    value={fuelSuppliers.find(f => f.supplier_name === form.supplierName)?.id || ""}
                    onChange={(id, label) => {
                      const supplier = fuelSuppliers.find(f => f.id === id);
                      set("supplierName")(supplier?.supplier_name || label.split(' - ')[0]);
                    }}
                  ...
                  <>
                    <SearchableCombobox
                      ...
                      value={fornecedoresShare.find(f => f.nome_completo === form.supplierName)?.id || form.supplierName}
                      onChange={(id, label) => {
                        const fornecedor = fornecedoresShare.find(f => f.id === id);
                        if (fornecedor) {
                          set("supplierName")(fornecedor.nome_completo);
                        } else {
                          set("supplierName")(id); // for free text
                        }
                      }}
