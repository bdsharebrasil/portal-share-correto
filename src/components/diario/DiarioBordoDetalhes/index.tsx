...
import { ExportDiarioModal } from "./components/ExportDiarioModal";
import { ConsumoDialog } from "./components/ConsumoDialog";
...
type Lanc = {
  ...
  // confirmado_por removido — coluna não existe no banco
...
function DiarioBordoDetalhes() {
  ...
  const reload = async () => {
    ...
    const [aRes, dmRes, lRes, lAnoRes, cRes, sRes, tRes, abRes, logbookIdsRes, aeroRes] = await Promise.all([
      ...
      supabase.from("aerodromes").select("id,designativo,nome,coordenadas").order("designativo"),
    ]);
  ...
  const handleExportarPDF = async (mesesSelecionados: Array<{ mes: number; ano: number }>) => {
    setIsExporting(true);
    const mesesNomes = mesesSelecionados.map(m => {
      const monthNames = ["janeiro", "fevereiro", "março", "abril", "maio", "junho", "julho", "agosto", "setembro", "outubro", "novembro", "dezembro"];
      return `${monthNames[m.mes - 1]} de ${m.ano}`;
    }).join(", ");

    try {
      if (!mesesSelecionados || mesesSelecionados.length === 0) {
        throw new Error("Nenhum mês foi selecionado para exportação");
      }

      if (!aeronave) {
        throw new Error("Dados da aeronave não encontrados");
      }
      ...
      for (const { mes: mês, ano: year } of mesesSelecionados) {
        try {
          ...
          mesesdados.push({
            ...
            totals: totaisData,
            porCotista: Array.from(mapCotista.values()).sort((a, b) => b.horas - a.horas),
            temDiaria: diarioMes?.tem_tarifa_diaria === true
          });
        } catch (monthError) {
          throw new Error(`Erro ao processar ${mesesNomes}: ${monthError instanceof Error ? monthError.message : String(monthError)}`);
        }
      }

      if (mesesdados.length === 0) {
        throw new Error("Nenhum dado foi processado para os meses selecionados");
      }

      const pdf = await exportDiarioBordoPDF({
        aeronave: { matricula: aeronave.matricula, modelo: aeronave.modelo, ano: aeronave.ano },
        meses: mesesdados
      }, "/share.png");

      pdf.save(`Diario_${aeronave.matricula}_${new Date().getTime()}.pdf`);
      toast.success(`✅ PDF exportado com sucesso! (${mesesNomes})`);
      setShowExportModal(false);
    } catch (error) {
      const errorMsg = error instanceof Error ? error.message : "Erro desconhecido ao exportar PDF";
      console.error("Erro em handleExportarPDF:", error);
      toast.error(`❌ Erro ao exportar PDF (${mesesNomes}): ${errorMsg}`);
    }
    finally {
                ...
                <table className="w-full border-collapse text-xs [&_td]:border-r [&_td]:border-slate-700/50 [&_th]:border-r [&_th]:border-slate-700/50 [&_td:last-child]:border-r-0 [&_th:last-child]:border-r-0" style={{ tableLayout: "fixed" }}>
                  <thead className="border-b border-slate-700/50 bg-slate-800/50 sticky top-0 z-10">
                    <tr className="text-[10px] uppercase tracking-wide text-slate-500">
                      ...
                      <th className="px-2 py-2 text-left relative" style={{ width: colWidths["voopara"] ?? 110 }}>
                        VOO PARA
                        <ResizeHandle col="voopara" />
                      </th>
                      ...
                      {/* CONF. POR removido — coluna não existe; exibe data de confirmação */}
                      <th className="px-2 py-2 text-center relative" style={{ width: colWidths["confEm"] ?? 80 }}>CONF. EM<ResizeHandle col="confEm" /></th>
      ...
      <div className="p-5 space-y-5 max-h-[70vh] overflow-y-auto">
        ...
        <Section title="Aeródromo">
          ...
          <Field label="Destino (ICAO)">
            <SearchableCombobox items={aerodromes.map((ad, idx) => ({ id: idx.toString(), label: `${ad.designativo} - ${ad.name}` }))} value={(() => { const idx = aerodromes.findIndex(ad => ad.designativo === destino); return idx >= 0 ? idx.toString() : ""; })()} onChange={(id, label) => setDestino(label.split(' - ')[0])} placeholder="Buscar..." searchPlaceholder="Código ou nome..." allowFreeText={true} />
          </Field>
        </Section>
