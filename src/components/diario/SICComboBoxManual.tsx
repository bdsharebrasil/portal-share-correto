...
export function SICComboBoxManual({
  ...
  const filteredCrew = useMemo(() => {
    ...
    return crew.filter(c =>
      c.nome_completo.toLowerCase().includes(searchValue.toLowerCase()) ||
  ...
  const displayText = useMemo(() => {
    ...
    if (normalizedValue) {
      ...
      return selected ? `${selected.nome_completo} (${selected.canac})` : placeholder;
    }
    return placeholder;
                    ...
                    {filteredCrew.length > 0 && (
                      <CommandGroup heading="Pilotos cadastrados">
                        {filteredCrew.map((crew_member) => (
                          <CommandItem
                            key={crew_member.id}
                            value={`${crew_member.nome_completo} ${crew_member.canac}`}
                          ...
                          >
                            ...
                            <div className="flex flex-col gap-0.5 flex-1">
                              <span className="font-medium text-white">{crew_member.nome_completo}</span>
