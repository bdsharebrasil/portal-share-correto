  ...
  const { data: clients = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return clientAircraft.map((ca: any) => ({
        ...
        share_percentage: ca.percentual_participacao,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (flightCategory === 'emprestimo' && insertedEntry) {
        ...
        const picName = selectedPic ? (allCrew.find(p => p.id === selectedPic)?.nome_completo || null) : null;
              ...
              <div className="space-y-2">
                ...
                <Popover open={borrowerClientOpen} onOpenChange={setBorrowerClientOpen}>
                  <PopoverTrigger asChild>
                    ...
                    >
                      {selectedBorrowerClient
                        ? borrowerClients.find(c => c.id === selectedBorrowerClient)?.razao_social || allClients.find(c => c.id === selectedBorrowerClient)?.razao_social || 'Cliente selecionado'
                    ...
                    >
                      {selectedPic
                        ? (() => {
                            ...
                            return pic ? `${pic.nome_completo} (${pic.canac})` : 'PIC selecionado';
                          })()
                        : 'Selecione o PIC...'
                      }
                    </Button>
                      ...
                      <CommandList>
                        ...
                        <CommandGroup>
                          {allCrew.map((tripulante) => (
                            <CommandItem
                              key={tripulante.id}
                              value={`${tripulante.nome_completo} ${tripulante.canac}`}
                              onSelect={() => {
                            ...
                            >
                              ...
                              <div className="flex flex-col gap-0.5">
                                <span className="font-medium">{tripulante.nome_completo}</span>
            ...
            <div className="grid grid-cols-3 gap-4 text-sm">
              ...
              <div>
                ...
                <p className="font-medium text-xs">
                  {selectedPic
                    ? allCrew.find(t => t.id === selectedPic)?.nome_completo?.split(' ')[0] || 'PIC'
                    : '-'
                  }
                </p>
              </div>
