    ...
    if (!existing) {
      const { data: cbData, error: cbError } = await supabase
        ...
        .insert({
          ...
          valor: rec.valor,
          referencia,
          status: mapReconciliationStatusToFluxo(rec.status, rec.tipo),
          criado_por: userId,
...
// Função auxiliar para buscar categoria_id
  ...
  try {
    ...
    if (!data) {
      ...
      if (createError) {
        ...
        console.error(`📋 Você pode resolver isso executando o script CREATE_SALARY_CATEGORIES.sql no SQL Editor do Supabase`);
        return null;
      }
    ...
    console.error(`📋 Dica: Verifique se a tabela "categorias_movimentacao" existe e se você tem permissão de acesso`);
    return null;
  }
}
