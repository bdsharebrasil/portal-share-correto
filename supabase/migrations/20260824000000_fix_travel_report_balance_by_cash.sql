...
DECLARE
  ...
  cash_type text := lower(trim(COALESCE(NEW.tipo_caixa, 'share')));
BEGIN
  ...
  SELECT CASE cash_type
    WHEN 'dga' THEN COALESCE(NULLIF(total_tripulacao, 0), NULLIF(total_trip, 0), 0)
    WHEN 'cliente' THEN COALESCE(NULLIF(total_clientes, 0), 0)
    ELSE COALESCE(NULLIF(total_sharebrasil, 0), 0)
  END
    INTO report_limit
  ...
  WHERE m.reference_type IN ('relatorio_viagem', 'travel_report')
    ...
    AND lower(trim(COALESCE(m.tipo_caixa, 'share'))) = cash_type
...
COMMENT ON FUNCTION public.validate_travel_report_movement_limit() IS
  'Bloqueia lançamentos de movimentacoes vinculados a relatório conforme o limite do respectivo caixa.';
